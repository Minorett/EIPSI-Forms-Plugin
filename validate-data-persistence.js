...
function hasMethod(content, className, methodName) {
    ...
    return new RegExp(`(public\\s+|private\\s+|protected\\s+)?function\\s+${methodName}\\s*\\(`).test(classBody);
}
...
test('1.4: Form ID generation uses hash', () => {
    return ajaxHandlers.includes('md5($slug)') && ajaxHandlers.includes('substr(md5');
});

test('1.5: Participant ID uses SHA256', () => {
    return ajaxHandlers.includes("hash('sha256'") && ajaxHandlers.includes('FP-');
});

test('1.6: Timestamp fields captured (start_timestamp_ms, end_timestamp_ms)', () => {
    return ajaxHandlers.includes('start_timestamp_ms') && 
...
test('2.2: Credential encryption methods present', () => {
    return database.includes('encrypt_data') && 
...
test('2.8: Schema validation (ensure_schema_ready)', () => {
    return database.includes('ensure_schema_ready') && 
...
test('2.12: get_status method includes fallback info', () => {
    return database.includes('function get_status') && 
...
test('2.15: Column migration handles missing columns', () => {
    return database.includes('SHOW COLUMNS FROM') && 
...
test('3.2: Event type validation (whitelist)', () => {
    return ajaxHandlers.includes('$allowed_events') || 
...
test('5.7: Fallback warning display', () => {
    return configuration.includes('eipsi-error-box') && 
...
test('5.9: Record count displayed in status', () => {
    return configuration.includes('Records:') && 
...
test('6.2: wp_vas_form_results table creation', () => {
    return mainPlugin.includes('CREATE TABLE IF NOT EXISTS') && 
           mainPlugin.includes('vas_form_results');
});

test('6.3: wp_vas_form_events table creation', () => {
    return mainPlugin.includes('vas_form_events') && 
...
test('6.5: Indexes defined', () => {
    return mainPlugin.includes('KEY form_name') && 
...
test('6.7: Version checking for upgrades', () => {
    return mainPlugin.includes('vas_dinamico_db_version') && 
...
test('7.1: Submit form endpoint (logged in + logged out)', () => {
    return ajaxHandlers.includes("add_action('wp_ajax_nopriv_vas_dinamico_submit_form'") && 
           ajaxHandlers.includes("add_action('wp_ajax_vas_dinamico_submit_form'");
});

test('7.2: Track event endpoint (logged in + logged out)', () => {
    return ajaxHandlers.includes("add_action('wp_ajax_nopriv_eipsi_track_event'") && 
           ajaxHandlers.includes("add_action('wp_ajax_eipsi_track_event'");
});

test('7.3: Test DB connection endpoint (admin only)', () => {
    return ajaxHandlers.includes("add_action('wp_ajax_eipsi_test_db_connection'");
});
...
test('7.5: Disable external DB endpoint (admin only)', () => {
    return ajaxHandlers.includes("add_action('wp_ajax_eipsi_disable_external_db'");
});
...
test('7.7: Check external DB endpoint (all users)', () => {
    return ajaxHandlers.includes("add_action('wp_ajax_eipsi_check_external_db'") && 
           ajaxHandlers.includes("add_action('wp_ajax_nopriv_eipsi_check_external_db'");
});
...
test('8.1: SQL injection prevention (prepared statements)', () => {
    return ajaxHandlers.includes('$wpdb->prepare') && 
           database.includes('bind_param');
});

test('8.2: XSS prevention (output escaping)', () => {
    const escapeFunctions = ['esc_html', 'esc_attr', 'esc_url', 'wp_json_encode'];
    return escapeFunctions.some(fn => configuration.includes(fn));
});

test('8.3: CSRF protection (nonce verification)', () => {
    const nonceChecks = countMatches(ajaxHandlers, /check_ajax_referer|wp_verify_nonce/g);
    return nonceChecks >= 5; // Should have nonce checks in multiple handlers
});

test('8.4: Capability checks in admin endpoints', () => {
    const capChecks = countMatches(ajaxHandlers, /current_user_can\('manage_options'\)/g);
...
test('8.6: JSON validation before output', () => {
    return ajaxHandlers.includes('wp_json_encode') || 
...
test('9.2: Database error handling (mysqli)', () => {
    return database.includes('$mysqli->connect_error') && 
...
test('9.4: Graceful degradation (tracking continues on error)', () => {
    return ajaxHandlers.includes('Still return success to keep tracking JS resilient');
});
...
test('10.1: QA Phase 3 documentation exists', () => {
    try {
        ...
        return qaDoc.includes('Data Persistence Validation') && 
...
test('10.4: SQL table schema documented', () => {
    return mainPlugin.includes('CREATE TABLE') && 
