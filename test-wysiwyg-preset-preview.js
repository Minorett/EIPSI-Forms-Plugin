...
editorFiles.forEach((filePath) => {
	...
	test(`${fileName}: Uses CSS variables (found ${cssVarCount})`, cssVarCount > 0);
	...
	test(
		...
		content.includes('--eipsi-color-background') || content.includes('--eipsi-color-background-subtle')
	);
	
	// Check for typography variables
	test(
		`${fileName}: Uses typography variables`,
		content.includes('--eipsi-font-') || content.includes('font-family: var(')
	);
...
if (stylePresetsContent) {
	...
	test(
		...
		presets.every((preset) =>
			...
			stylePresetsContent.match(new RegExp(`STYLE_PRESETS[^]*${preset}`))
		)
	);
...
if (buildExists) {
	...
	const hasEditorAssets = buildFiles.some(file =>
...
if (readmeContent) {
	...
	test(
		...
		readmeContent.toLowerCase().includes('what you see is what you get') ||
