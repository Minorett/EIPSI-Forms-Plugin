...
console.log(`${COLORS.BOLD}${COLORS.BLUE}║   PRIVACY TOGGLES VALIDATION                          ║${COLORS.RESET}`);
console.log(`${COLORS.BOLD}${COLORS.BLUE}║   Browser/OS/Screen Width/IP Privacy Config           ║${COLORS.RESET}`);
console.log(`${COLORS.BOLD}${COLORS.BLUE}╚═══════════════════════════════════════════════════════╝${COLORS.RESET}`);
...
test(
    ...
    privacyConfig && privacyConfig.match(/\$allowed_toggles[\s\S]*?'browser'/),
...
test(
    ...
    privacyDashboard && !privacyDashboard.match(/name="ip_address"[^>]*disabled/),
    'Expected: IP Address checkbox not disabled'
);

test(
    'Browser defaults to unchecked (false)',
    privacyDashboard && privacyDashboard.match(/name="browser".*checked\(\$privacy_config\['browser'\] \?\? false\)/),
    'Expected: Browser defaults to false'
);

test(
    'OS defaults to unchecked (false)',
    privacyDashboard && privacyDashboard.match(/name="os".*checked\(\$privacy_config\['os'\] \?\? false\)/),
    'Expected: OS defaults to false'
);

test(
    'Screen Width defaults to unchecked (false)',
    privacyDashboard && privacyDashboard.match(/name="screen_width".*checked\(\$privacy_config\['screen_width'\] \?\? false\)/),
    'Expected: Screen Width defaults to false'
);

test(
    'IP Address defaults to checked (true)',
    privacyDashboard && privacyDashboard.match(/name="ip_address".*checked\(\$privacy_config\['ip_address'\] \?\? true\)/),
    'Expected: IP Address defaults to true'
);

test(
    'Device Info section exists with "Opcional" label',
    privacyDashboard && privacyDashboard.includes('🖥️ Información de Dispositivo') && privacyDashboard.includes('eipsi-optional'),
    'Expected: Device Info section with optional styling'
);

test(
    'Section description warning exists',
    privacyDashboard && privacyDashboard.includes('eipsi-section-description') && privacyDashboard.includes('desactivados por defecto'),
...
test(
    ...
    privacyDashboard && privacyDashboard.includes('.eipsi-section-description'),
    'Expected: CSS styling for section description'
);

test(
    'Updated info box exists',
    privacyDashboard && privacyDashboard.includes('Por defecto ON - Auditoría clínica') && privacyDashboard.includes('Por defecto OFF - Solo para debugging'),
...
test(
    ...
    ajaxHandlers && ajaxHandlers.includes('$browser_raw') && ajaxHandlers.includes("$_POST['browser']"),
    'Expected: $browser_raw captured from $_POST'
);

test(
    'OS_raw is captured from POST',
    ajaxHandlers && ajaxHandlers.includes('$os_raw') && ajaxHandlers.includes("$_POST['os']"),
    'Expected: $os_raw captured from $_POST'
);

test(
    'Screen Width_raw is captured from POST',
    ajaxHandlers && ajaxHandlers.includes('$screen_width_raw') && ajaxHandlers.includes("$_POST['screen_width']"),
    'Expected: $screen_width_raw captured from $_POST'
);

test(
    'IP Address_raw is captured from SERVER',
    ajaxHandlers && ajaxHandlers.includes('$ip_address_raw') && ajaxHandlers.includes("$_SERVER['REMOTE_ADDR']"),
    'Expected: $ip_address_raw captured from $_SERVER'
);

test(
    'Browser respects privacy config',
    ajaxHandlers && ajaxHandlers.match(/\$browser\s*=\s*\(\$privacy_config\['browser'\].*\?\s*\$browser_raw\s*:\s*null/),
    'Expected: $browser set to null if privacy config is false'
);

test(
    'OS respects privacy config',
    ajaxHandlers && ajaxHandlers.match(/\$os\s*=\s*\(\$privacy_config\['os'\].*\?\s*\$os_raw\s*:\s*null/),
    'Expected: $os set to null if privacy config is false'
);

test(
    'Screen Width respects privacy config',
    ajaxHandlers && ajaxHandlers.match(/\$screen_width\s*=\s*\(\$privacy_config\['screen_width'\].*\?\s*\$screen_width_raw\s*:\s*null/),
    'Expected: $screen_width set to null if privacy config is false'
);

test(
    'IP Address respects privacy config',
    ajaxHandlers && ajaxHandlers.match(/\$ip_address\s*=\s*\(\$privacy_config\['ip_address'\].*\?\s*\$ip_address_raw\s*:\s*null/),
    'Expected: $ip_address set to null if privacy config is false'
);

test(
    'Browser is added to device_info metadata',
    ajaxHandlers && ajaxHandlers.includes("if ($browser !== null)") && ajaxHandlers.includes("$device_info['browser']"),
    'Expected: Browser conditionally added to metadata'
);

test(
    'OS is added to device_info metadata',
    ajaxHandlers && ajaxHandlers.includes("if ($os !== null)") && ajaxHandlers.includes("$device_info['os']"),
    'Expected: OS conditionally added to metadata'
);

test(
    'Screen Width is added to device_info metadata',
    ajaxHandlers && ajaxHandlers.includes("if ($screen_width !== null)") && ajaxHandlers.includes("$device_info['screen_width']"),
    'Expected: Screen Width conditionally added to metadata'
);

test(
    'IP Address is conditionally added to network_info',
    ajaxHandlers && ajaxHandlers.includes("if ($ip_address !== null)") && ajaxHandlers.includes("$metadata['network_info']"),
...
if (failed === 0) {
    console.log(`${COLORS.GREEN}${COLORS.BOLD}✓ ALL TESTS PASSED (${passed}/${total})${COLORS.RESET}`);
    console.log(`${COLORS.GREEN}${COLORS.BOLD}✓ Privacy toggles implementation complete!${COLORS.RESET}\n`);
    process.exit(0);
} else {
