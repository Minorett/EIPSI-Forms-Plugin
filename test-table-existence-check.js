...
assert( fileExists( databasePhpPath ), 'admin/database.php exists' );
...
assert( fileExists( ajaxHandlersPath ), 'admin/ajax-handlers.php exists' );

assert(
	fileContains(
		ajaxHandlersPath,
		"add_action('wp_ajax_eipsi_check_table_status'"
	),
...
const configurationPhpPath = path.join(
	__dirname,
	'admin',
	'configuration.php'
);

assert( fileExists( configurationPhpPath ), 'admin/configuration.php exists' );
...
const configPanelJsPath = path.join(
	__dirname,
	'assets',
	'js',
...
const configPanelCssPath = path.join(
	__dirname,
	'assets',
	'css',
...
assert(
	fileContains( configPanelCssPath, '#198754' ) ||
		fileContains( configPanelCssPath, 'var(--eipsi-color-success' ),
	'CSS uses WCAG AA compliant success color'
);

assert(
	fileContains( configPanelCssPath, '#b35900' ) ||
		fileContains( configPanelCssPath, 'var(--eipsi-color-warning' ),
	'CSS uses WCAG AA compliant warning color'
);

assert(
	fileContains( configPanelCssPath, '#d32f2f' ) ||
		fileContains( configPanelCssPath, 'var(--eipsi-color-error' ),
...
assert(
	fileContains( configPanelCssPath, '.eipsi-table-status-box' ) &&
		fileContainsPattern(
			configPanelCssPath,
			/@media.*max-width.*374px[\s\S]*\.eipsi-table-status-box/
		),
	'CSS includes mobile styles for table status box'
);

assert(
	fileContains( configPanelCssPath, 'padding: 0.75rem' ) ||
		fileContains( configPanelCssPath, 'padding: 1rem' ),
...
assert(
	fileContains( databasePhpPath, '✓' ) ||
		fileContains( databasePhpPath, '⚠️' ),
...
assert(
	fileContains( databasePhpPath, '__(' ) ||
		fileContains( databasePhpPath, 'esc_' ),
...
assert(
	fileContains(
		configurationPhpPath,
		'Configure and connect to an external database'
	),
...
if ( failedTests > 0 ) {
	...
	console.log( '' );
	console.log(
		'🎉 All tests passed! Table existence check feature is complete.'
	);
	process.exit( 0 );
}
