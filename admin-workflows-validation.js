...
function test(category, name, fn) {
    ...
    try {
        ...
        if (result === true) {
            ...
            console.log(`${colors.green}✓${colors.reset} ${colors.gray}[${category}]${colors.reset} ${name}`);
            results.details.push({ category, name, status: 'PASS' });
        } else if (result === 'warning') {
            results.warnings++;
            results.categories[category].passed++;
            console.log(`${colors.yellow}⚠${colors.reset} ${colors.gray}[${category}]${colors.reset} ${name}`);
            results.details.push({ category, name, status: 'WARNING', message: result.message || '' });
        } else {
            results.failed++;
            results.categories[category].failed++;
            console.log(`${colors.red}✗${colors.reset} ${colors.gray}[${category}]${colors.reset} ${name}`);
            results.details.push({ category, name, status: 'FAIL', message: result });
        }
    } catch (error) {
        results.failed++;
        results.categories[category].failed++;
        ...
        results.details.push({ category, name, status: 'FAIL', message: error.message });
    }
}
...
console.log(colors.bright + '  EIPSI Forms - Admin Workflows Validation (Phase 7)' + colors.reset);
console.log(colors.cyan + '━'.repeat(80) + colors.reset + '\n');

console.log(`${colors.gray}Testing all admin-side functionality...${colors.reset}\n`);
...
test('Block Editor', 'Form Container imports InspectorControls', () => {
    return containsPattern('src/blocks/form-container/edit.js', /InspectorControls/);
});
...
test('Block Editor', 'Form Container has allowBackwardsNav toggle', () => {
    return containsPattern('src/blocks/form-container/edit.js', /allowBackwardsNav/);
});

test('Block Editor', 'Form Container has description textarea', () => {
    return containsPattern('src/blocks/form-container/edit.js', /description/);
});

test('Block Editor', 'Form Container imports FormStylePanel', () => {
    return containsPattern('src/blocks/form-container/edit.js', /FormStylePanel/);
});
...
test('Block Editor', 'FormStylePanel has color picker controls', () => {
    return containsPattern('src/components/FormStylePanel.js', /ColorPalette/);
});

test('Block Editor', 'FormStylePanel has spacing controls', () => {
    return containsPattern('src/components/FormStylePanel.js', /RangeControl/);
});
...
test('Block Editor', 'styleTokens has serializeToCSSVariables function', () => {
    return containsPattern('src/utils/styleTokens.js', /serializeToCSSVariables/);
});
...
test('Block Editor', 'Form Container has allowed blocks list', () => {
    return containsPattern('src/blocks/form-container/edit.js', /ALLOWED_BLOCKS/);
});
...
test('Block Editor', 'Form fields blocks exist (7 types)', () => {
    ...
    return fields.every(field => fileExists(`src/blocks/${field}/edit.js`));
});
...
test('Results Page', 'Has dynamic colspan calculation', () => {
    return containsPattern('admin/results-page.php', /\$colspan = \$show_form_column \? 8 : 7/);
});

test('Results Page', 'Displays privacy notice about metadata-only view', () => {
...
test('Results Page', 'Has View response button with data-id attribute', () => {
    return containsPattern('admin/results-page.php', /vas-view-response.*data-id/s);
});

test('Results Page', 'Has Delete response button with nonce URL', () => {
    return containsPattern('admin/results-page.php', /wp_nonce_url.*delete_response_/s);
});
...
test('Results Page', 'Has AJAX call to eipsi_get_response_details', () => {
    return containsPattern('admin/results-page.php', /eipsi_get_response_details/);
});

test('Results Page', 'Has research context toggle button', () => {
    return containsPattern('admin/results-page.php', /toggle-research-context/);
});

test('Results Page', 'Formats date/time with WordPress timezone', () => {
    return containsPattern('admin/results-page.php', /timezone_string.*DateTimeZone/s);
});

test('Results Page', 'Uses duration_seconds with fallback to duration', () => {
    return containsPattern('admin/results-page.php', /duration_seconds.*duration/);
});
...
test('Results Page', 'Has delete success/error notice handling', () => {
    return containsPattern('admin/results-page.php', /deleted.*notice-success/s);
});

test('Results Page', 'Has nonce creation for AJAX', () => {
    return containsPattern('admin/results-page.php', /wp_create_nonce.*eipsi_admin_nonce/);
});
...
test('Configuration Panel', 'Has database indicator banner', () => {
    return containsPattern('admin/configuration.php', /eipsi-db-indicator-banner/);
});

test('Configuration Panel', 'Shows external vs WordPress database badge', () => {
    return containsPattern('admin/configuration.php', /eipsi-db-badge--external.*eipsi-db-badge--wordpress/s);
});
...
test('Configuration Panel', 'Has password input field with conditional required', () => {
    return containsPattern('admin/configuration.php', /db_password.*credentials \? '' : 'required'/s);
});
...
test('Configuration Panel', 'Has Test Connection button', () => {
    return containsPattern('admin/configuration.php', /eipsi-test-connection/);
});

test('Configuration Panel', 'Has Save Configuration button (disabled by default)', () => {
    return containsPattern('admin/configuration.php', /eipsi-save-config.*disabled/s);
});

test('Configuration Panel', 'Has Disable External Database button', () => {
    return containsPattern('admin/configuration.php', /eipsi-disable-external-db/);
});

test('Configuration Panel', 'Has message container with role="alert"', () => {
    return containsPattern('admin/configuration.php', /eipsi-message-container.*role="alert"/s);
});
...
test('Configuration Panel', 'Displays record count and last updated', () => {
    return containsPattern('admin/configuration.php', /record_count.*last_updated/s);
});
...
test('Configuration Panel', 'Requires manage_options capability', () => {
    return containsPattern('admin/configuration.php', /current_user_can.*manage_options/);
});
...
test('Export Functionality', 'Excel export has timestamp columns', () => {
    return containsPattern('admin/export.php', /'Start Time \(UTC\)'.*'End Time \(UTC\)'/s);
});

test('Export Functionality', 'Uses duration_seconds with fallback', () => {
    return containsPattern('admin/export.php', /duration_seconds.*number_format.*duration/s);
});

test('Export Functionality', 'Generates stable form ID from form name', () => {
    return containsPattern('admin/export.php', /export_generate_stable_form_id/);
});

test('Export Functionality', 'Generates stable participant fingerprint', () => {
    return containsPattern('admin/export.php', /export_generateStableFingerprint/);
});
...
test('Export Functionality', 'Requires manage_options capability', () => {
    return containsPattern('admin/export.php', /current_user_can.*manage_options/);
});
...
test('AJAX Handlers', 'Registers vas_dinamico_submit_form handler', () => {
    return containsPattern('admin/ajax-handlers.php', /add_action.*vas_dinamico_submit_form/);
});

test('AJAX Handlers', 'Registers eipsi_get_response_details handler', () => {
    return containsPattern('admin/ajax-handlers.php', /add_action.*eipsi_get_response_details/);
});

test('AJAX Handlers', 'Registers eipsi_track_event handler', () => {
    return containsPattern('admin/ajax-handlers.php', /add_action.*eipsi_track_event/);
});

test('AJAX Handlers', 'Registers eipsi_test_db_connection handler', () => {
    return containsPattern('admin/ajax-handlers.php', /add_action.*eipsi_test_db_connection/);
});

test('AJAX Handlers', 'Registers eipsi_save_db_config handler', () => {
    return containsPattern('admin/ajax-handlers.php', /add_action.*eipsi_save_db_config/);
});

test('AJAX Handlers', 'Registers eipsi_disable_external_db handler', () => {
    return containsPattern('admin/ajax-handlers.php', /add_action.*eipsi_disable_external_db/);
});
...
test('AJAX Handlers', 'Generates stable form ID in submission', () => {
    return containsPattern('admin/ajax-handlers.php', /generate_stable_form_id/);
});

test('AJAX Handlers', 'Generates participant fingerprint in submission', () => {
    return containsPattern('admin/ajax-handlers.php', /generateStableFingerprint/);
});

test('AJAX Handlers', 'Calculates duration from timestamps', () => {
    return containsPattern('admin/ajax-handlers.php', /duration_seconds.*start_timestamp_ms.*end_timestamp_ms/s);
});

test('AJAX Handlers', 'Sanitizes all POST data', () => {
    const count = countOccurrences('admin/ajax-handlers.php', /sanitize_text_field|sanitize_email|intval/g);
    return count >= 10; // Should have many sanitization calls
});

test('AJAX Handlers', 'Has ABSPATH security check', () => {
    return containsPattern('admin/ajax-handlers.php', /if \(!defined\('ABSPATH'\)\)/);
});
...
test('Admin Assets', 'configuration-panel.js has EIPSIConfig object', () => {
    return containsPattern('assets/js/configuration-panel.js', /const EIPSIConfig/);
});

test('Admin Assets', 'configuration-panel.js has testConnection method', () => {
    return containsPattern('assets/js/configuration-panel.js', /testConnection/);
});

test('Admin Assets', 'configuration-panel.js has saveConfiguration method', () => {
    return containsPattern('assets/js/configuration-panel.js', /saveConfiguration/);
});

test('Admin Assets', 'configuration-panel.js has disableExternalDB method', () => {
    return containsPattern('assets/js/configuration-panel.js', /disableExternalDB/);
});
...
test('Admin Assets', 'configuration-panel.js has updateStatusBox method', () => {
    return containsPattern('assets/js/configuration-panel.js', /updateStatusBox/);
});

test('Admin Assets', 'configuration-panel.js disables save until test passes', () => {
    return containsPattern('assets/js/configuration-panel.js', /connectionTested = false.*save-config.*disabled/s);
});

test('Admin Assets', 'configuration-panel.js clears password after save', () => {
    return containsPattern('assets/js/configuration-panel.js', /#db_password.*val\( '' \)/s);
});

test('Admin Assets', 'configuration-panel.js has loading state handling', () => {
    return containsPattern('assets/js/configuration-panel.js', /eipsi-loading/);
});

test('Admin Assets', 'configuration-panel.js auto-hides success messages', () => {
    return containsPattern('assets/js/configuration-panel.js', /setTimeout.*fadeOut/s);
});
...
test('Admin Assets', 'configuration-panel.css has status indicator styles', () => {
    return containsPattern('assets/css/configuration-panel.css', /status-icon|status-connected|status-disconnected/);
});
...
test('Security & Validation', 'Results page uses wp_nonce_url for delete', () => {
    return containsPattern('admin/results-page.php', /wp_nonce_url/);
});

test('Security & Validation', 'Results page has confirmation dialog for delete', () => {
    return containsPattern('admin/results-page.php', /confirm.*delete this response/s);
});
...
test('Security & Validation', 'Export functions check manage_options capability', () => {
    const content = readFile('admin/export.php');
    const matches = content.match(/current_user_can\('manage_options'\)/g);
    return matches && matches.length >= 2; // Both export functions
});

test('Security & Validation', 'AJAX handlers use nonce verification', () => {
    return containsPattern('admin/ajax-handlers.php', /check_ajax_referer|wp_verify_nonce/);
});

test('Security & Validation', 'Configuration requires manage_options', () => {
    return containsPattern('admin/configuration.php', /current_user_can.*manage_options.*wp_die/s);
});

test('Security & Validation', 'All form inputs are sanitized', () => {
    const count = countOccurrences('admin/ajax-handlers.php', /sanitize_text_field|sanitize_email|esc_attr|esc_html/g);
    return count >= 15; // Should have many sanitization calls
});

test('Security & Validation', 'Database queries use prepared statements', () => {
    const content = readFile('admin/ajax-handlers.php');
    const prepare = (content.match(/\$wpdb->prepare/g) || []).length;
    const direct = (content.match(/\$wpdb->query\(\s*"SELECT|INSERT|UPDATE|DELETE/g) || []).length;
...
test('Security & Validation', 'Output is escaped in results page', () => {
    const count = countOccurrences('admin/results-page.php', /esc_html|esc_attr|esc_url/g);
...
test('Security & Validation', 'AJAX responses use wp_send_json_*', () => {
    return containsPattern('admin/ajax-handlers.php', /wp_send_json_success|wp_send_json_error/);
});

test('Security & Validation', 'Delete action validates nonce with response ID', () => {
    return containsPattern('admin/results-page.php', /delete_response_.*\$row->id/s);
});
...
const overallColor = results.failed === 0 ? colors.green : (results.failed < 5 ? colors.yellow : colors.red);

console.log(colors.bright + 'Overall Results:\n' + colors.reset);
console.log(`  ${colors.green}Passed:  ${colors.bright}${results.passed}${colors.reset}`);
console.log(`  ${colors.red}Failed:  ${colors.bright}${results.failed}${colors.reset}`);
console.log(`  ${colors.yellow}Warnings: ${colors.bright}${results.warnings}${colors.reset}`);
console.log(`  Total:   ${colors.bright}${results.total}${colors.reset}`);
console.log(`  ${overallColor}Pass Rate: ${colors.bright}${passRate}%${colors.reset}\n`);

// Status badge
if (results.failed === 0) {
    console.log(`  ${colors.green}${colors.bright}✓ ALL TESTS PASSED${colors.reset}\n`);
} else if (results.failed < 5) {
    console.log(`  ${colors.yellow}${colors.bright}⚠ MOSTLY PASSING (Minor Issues)${colors.reset}\n`);
} else {
...
console.log(`${colors.gray}Results saved to: docs/qa/admin-workflows-validation.json${colors.reset}\n`);
