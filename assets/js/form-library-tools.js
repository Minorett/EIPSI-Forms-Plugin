...
( function ( $ ) {
    ...
    const FormLibraryTools = {
        ...
        showExportModeModal( templateId, templateName, $triggerLink ) {
            ...
            $modal
                .find(
                    '.eipsi-import-modal-close, .eipsi-export-mode-cancel, .eipsi-import-modal-backdrop'
                )
                .off( 'click' )
                .on( 'click', function () {
            ...
            // Confirm export
            $modal
                .find( '.eipsi-export-mode-confirm' )
                .off( 'click' )
                .on( 'click', function () {
                    const mode = $modal
                        .find( 'input[name="export_mode"]:checked' )
                        .val();
                    $modal.fadeOut( 200, function () {
                        $( this ).remove();
                    } );
        ...
        bindDuplicateActions() {
            $( document ).on( 'click', '.eipsi-duplicate-form', function ( e ) {
                ...
                if (
                    ! confirm(
                        eipsiFormTools.strings.duplicateConfirm +
                            '\n\n' +
                            templateName
                    )
                ) {
        ...
        bindClinicalTemplatesButtons() {
            $( document ).on(
                ...
                function ( e ) {
                    ...
                    const templateName =
                        $button.data( 'template-name' ) || templateId;
                    ...
                    const confirmTemplate =
                        eipsiFormTools.strings.clinicalTemplateConfirm || '';
                    const confirmMessage = confirmTemplate
                        ? confirmTemplate.replace( '%s', templateName )
                        : '';
                        ...
                        success( response ) {
                            if ( response.success ) {
                                ...
                                FormLibraryTools.showNotice(
                                    ( response.data &&
                                        response.data.message ) ||
                                        eipsiFormTools.strings
                                            .clinicalTemplateError,
                                    'error'
                                );
        ...
        bindImportModalActions() {
            ...
            // Close modal
            $(
                '.eipsi-import-modal-close, .eipsi-import-cancel, .eipsi-import-modal-backdrop'
            )
                .off( 'click' )
                .on( 'click', function () {
            ...
            $submitBtn.off( 'click' ).on( 'click', function () {
                if ( FormLibraryTools.selectedFile ) {
                    FormLibraryTools.processImport(
                        FormLibraryTools.selectedFile
                    );
                }
            } );
        },
        ...
        handleFileSelect( file ) {
            ...
            $error.hide();
            $filename
                .html( `<strong>Archivo seleccionado:</strong> ${ file.name }` )
                .show();
        ...
        processImport( file ) {
            ...
            reader.onload = function ( e ) {
                ...
                $.ajax( {
                    ...
                    success( response ) {
                        if ( response.success ) {
                            ...
                            $error
                                .html(
                                    response.data.message ||
                                        eipsiFormTools.strings.importError
                                )
                                .show();
                            $submitBtn
                                .text( originalText )
                    ...
                    error() {
                        $error
                            .html( eipsiFormTools.strings.importError )
                            .show();
                        $submitBtn
                            .text( originalText )
                            .prop( 'disabled', false );
        ...
        /**
         * Download JSON file
         * Validates that special characters (newlines, etc.) are preserved
         */
        downloadJSON( data, filename ) {
            // Validate that the JSON preserves special characters
            const jsonString = JSON.stringify( data, null, 2 );

            // Verify that newlines are properly escaped in the JSON string
            // This ensures round-trip preservation
            const hasNewlines = jsonString.includes( '\\n' );
            const hasEscapedChars = /\\[ntr\\"]/.test( jsonString );

            if ( ! hasNewlines && hasEscapedChars ) {
                console.warn(
                    'EIPSI Forms: JSON may not preserve newlines correctly'
                );
            }
