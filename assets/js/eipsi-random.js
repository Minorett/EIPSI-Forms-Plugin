...
( function () {
    ...
    async function initRandomization() {
        ...
        const mainForm = document.querySelector( '.vas-form, .vas-dinamico-form' );
        if ( ! mainForm ) {
            return;
        }
        ...
        try {
            const ajaxUrl = window.eipsiRandomData?.ajaxUrl || window.eipsiFormsConfig?.ajaxUrl || '/wp-admin/admin-ajax.php';
            const nonce = window.eipsiRandomData?.nonce || window.eipsiFormsConfig?.nonce || '';
            ...
            if ( data.success && data.data ) {
                const { form_id: formId, seed, type } = data.data;
