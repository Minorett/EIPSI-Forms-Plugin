...
runner.test( 'edit.js: TextareaControl label does NOT mention comma', () => {
	const filePath = '/home/engine/project/src/blocks/campo-multiple/edit.js';
	const content = readFile( filePath );
	const hasCommaSeparatedLabel = content.includes(
		"'Options (comma-separated)'"
	);
	if ( hasCommaSeparatedLabel ) {
		throw new Error(
			'TextareaControl should not have "comma-separated" label anymore'
		);
	}
} );

runner.test(
	'edit.js: TextareaControl value joins options with newline',
	() => {
		const filePath =
...
runner.test( 'block.json: Example uses newline separator', () => {
	...
	if (
		! parsed.example ||
		! parsed.example.attributes ||
		! parsed.example.attributes.options
	) {
	...
	if ( ! options.includes( '\n' ) ) {
		throw new Error(
			`block.json example should use newline separator. Got: ${ options }`
		);
	}
} );
...
runner.test( 'block.json: Example does NOT use comma separator', () => {
	...
	const options = parsed.example.attributes.options;
	const hasComma =
		options.split( ',' ).length > 1 && ! options.includes( '\n' );
	if ( hasComma ) {
		throw new Error(
			`block.json example should NOT use comma separator. Got: ${ options }`
		);
	}
} );
...
runner.test( 'block.json: Example shows Spanish options', () => {
	...
	if ( ! /[áéíóúñ]/i.test( options ) ) {
		throw new Error(
			`block.json example should show Spanish options. Got: ${ options }`
		);
	}
} );
...
runner.test( 'Backward Compatibility: Logic handles comma-only format', () => {
	...
	if ( result.length !== 3 ) {
		throw new Error(
			`Legacy comma parsing failed. Expected 3 options, got ${ result.length }`
		);
	}
	if ( result[ 0 ] !== 'Opción 1' ) {
		throw new Error(
			`Legacy comma parsing failed. Expected "Opción 1", got "${ result[ 0 ] }"`
		);
	}
} );
...
runner.test( 'Backward Compatibility: Logic handles newline format', () => {
	...
	if ( result.length !== 3 ) {
		throw new Error(
			`Newline parsing failed. Expected 3 options, got ${ result.length }`
		);
	}
	if ( result[ 0 ] !== 'Sí, absolutamente' ) {
		throw new Error(
			`Newline parsing failed. Expected "Sí, absolutamente", got "${ result[ 0 ] }"`
		);
	}
} );

runner.test(
	'Backward Compatibility: Options with commas are preserved',
	() => {
		const filePath =
			'/home/engine/project/src/blocks/campo-multiple/edit.js';
		const content = readFile( filePath );
		...
		if ( result.length !== 3 ) {
			throw new Error(
				`Comma preservation failed. Expected 3 options, got ${ result.length }`
			);
		}
		if ( ! result[ 0 ].includes( ',' ) ) {
			throw new Error(
				`Comma preservation failed. Option should contain comma: "${ result[ 0 ] }"`
			);
		}
		if ( result[ 0 ] !== 'Sí, absolutamente' ) {
			throw new Error(
				`Comma preservation failed. Expected "Sí, absolutamente", got "${ result[ 0 ] }"`
			);
		}
	}
);
