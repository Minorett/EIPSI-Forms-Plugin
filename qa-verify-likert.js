...
if ( fs.existsSync( formsJsPath ) ) {
	...
	if ( formsJsContent.includes( "addEventListener( 'change'" ) ) {
		checkPassed(
			'eipsi-forms.js uses "change" event (correct for radios)'
		);
	} else {
...
if ( fs.existsSync( formsJsPath ) ) {
	...
	const radioGroupPattern =
		/input\[type="radio"\]\[name="\$\{\s*field\.name\s*\}"\]/;
	...
	if (
		formsJsContent.includes( '.some(' ) &&
		formsJsContent.includes( 'radio.checked' )
	) {
	...
	if (
		formsJsContent.includes( 'strings.requiredField' ) ||
...
checkPassed(
	'Touch events work natively with radio inputs (no custom code needed)'
);
...
if ( fs.existsSync( testFilePath ) ) {
	...
	checkInfo(
		'To run manual test: Open http://localhost:8080/test-likert-fix.html'
	);
} else {
...
if ( allPassed ) {
	...
	console.log(
		'  4. Test validation - verify required field validation works'
	);
