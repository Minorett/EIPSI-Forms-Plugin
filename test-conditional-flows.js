...
/* eslint-disable no-console, no-unused-vars, no-useless-constructor */
...
( function () {
		...
		return {
			...
			querySelector( selector ) {
				return this.children.find( ( child ) => {
					if ( selector === 'select' && child.tagName === 'SELECT' ) {
						return true;
					}
					if (
			...
			querySelectorAll( selector ) {
				if ( selector === 'input[type="checkbox"]:checked' ) {
					return this.children.filter(
						( child ) => child.type === 'checkbox' && child.checked
					);
				}
				return [];
			},
	...
	assertEqual(
		match85?.targetPage,
		5,
		'Matched rule has correct target page (5)'
	);
	...
	assertEqual(
		match75?.targetPage,
		3,
		'Value 75 matches second rule (>= 50)'
	);
	...
	assertEqual(
		match80?.targetPage,
		5,
		'Boundary value 80 matches >= 80 rule'
	);
	...
	assertEqual(
		match50?.targetPage,
		3,
		'Boundary value 50 matches >= 50 rule'
	);
	...
	const testOperator = ( operator, threshold, testValue, shouldMatch ) => {
		...
		assertEqual(
			matched,
			shouldMatch,
			`${ operator } ${ threshold }: value ${ testValue } ${
				shouldMatch ? 'should' : 'should not'
			} match`
		);
	};
	...
	assertEqual(
		radioValue,
		'Yes',
		'Radio field returns correct string value'
	);
	...
	assertEqual(
		matchYes?.action,
		'goToPage',
		'Matched discrete rule has correct action'
	);
	...
	assertEqual(
		matchNumeric?.targetPage,
		10,
		'Numeric value matches numeric rule in mixed set'
	);

	const matchDiscrete = navigator.findMatchingRule( mixedRules, 'High Risk' );
	assertEqual(
		matchDiscrete?.targetPage,
		8,
		'Discrete value matches discrete rule in mixed set'
	);
	...
	assertEqual(
		matchString,
		null,
		'String value does not match numeric rule'
	);
	...
	const validJSON =
		'{"enabled":true,"rules":[{"id":"rule-1","operator":">=","threshold":50,"action":"goToPage","targetPage":3}]}';
	...
	assertEqual(
		normalized.enabled,
		true,
		'Normalized logic has enabled flag'
	);
	assertEqual(
		normalized.rules[ 0 ].matchValue,
		'Yes',
		'Normalized rule has matchValue'
	);
	...
	assertEqual(
		navigator.findMatchingRule( mentalHealthRules, 85 )?.targetPage,
		15,
		'Crisis level (85) → page 15'
	);
	assertEqual(
		navigator.findMatchingRule( mentalHealthRules, 70 )?.targetPage,
		10,
		'High severity (70) → page 10'
	);
	assertEqual(
		navigator.findMatchingRule( mentalHealthRules, 50 )?.targetPage,
		5,
		'Moderate severity (50) → page 5'
	);
	assertEqual(
		navigator.findMatchingRule( mentalHealthRules, 30 ),
		null,
		'Low severity (30) → default action'
	);
