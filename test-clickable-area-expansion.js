...
// Test Likert edit.js
const likertEditPath = path.join(__dirname, 'src/blocks/campo-likert/edit.js');
...
test('Likert edit.js: Label wraps both input and text span', () => {
    const pattern = /<label[^>]*className="likert-label-wrapper"[^>]*>[\s\S]*?<input[^>]*type="radio"[\s\S]*?<span className="likert-label-text">/;
    assert(
...
// Test Likert save.js
const likertSavePath = path.join(__dirname, 'src/blocks/campo-likert/save.js');
...
// Test Likert styles
const likertStylePath = path.join(__dirname, 'src/blocks/campo-likert/style.scss');
...
test('Likert style.scss: Hides input with position: absolute; opacity: 0', () => {
    const hiddenInputPattern = /input\[type="radio"\][^}]*{[^}]*position:\s*absolute[^}]*opacity:\s*0/s;
    assert(
...
const multipleEditPath = path.join(__dirname, 'src/blocks/campo-multiple/edit.js');
...
test('Multiple Choice edit.js: Uses <span className="checkbox-label-text"> for text', () => {
    assert(
        multipleEditContent.includes('<span className="checkbox-label-text">'),
        'No span with className="checkbox-label-text" found'
    );
});

test('Multiple Choice edit.js: Label wraps both input and text span', () => {
    const pattern = /<label[^>]*className="checkbox-label-wrapper"[^>]*>[\s\S]*?<input[^>]*type="checkbox"[\s\S]*?<span className="checkbox-label-text">/;
    assert(
...
const multipleSavePath = path.join(__dirname, 'src/blocks/campo-multiple/save.js');
...
test('Multiple Choice save.js: Uses <span className="checkbox-label-text"> for text', () => {
    assert(
        multipleSaveContent.includes('<span className="checkbox-label-text">'),
        'No span with className="checkbox-label-text" found'
    );
});

// Test Multiple Choice styles
const multipleStylePath = path.join(__dirname, 'src/blocks/campo-multiple/style.scss');
...
test('Multiple Choice style.scss: Hides input with position: absolute; opacity: 0', () => {
    const hiddenInputPattern = /input\[type="checkbox"\][^}]*{[^}]*position:\s*absolute[^}]*opacity:\s*0/s;
    assert(
...
test('Likert: Has hover state for better UX', () => {
    assert(
        likertStyleContent.includes(':hover'),
        'No hover state found'
    );
});
...
test('Multiple Choice: Has hover state for better UX', () => {
    assert(
        multipleStyleContent.includes(':hover'),
        'No hover state found'
    );
});
...
if (passedTests === totalTests) {
    ...
    log(COLORS.red, '✗', `TESTS FAILED: ${passedTests}/${totalTests} passed`);
    console.log('='.repeat(70));
    process.exit(1);
}
