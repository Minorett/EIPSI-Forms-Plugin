...
test('Old message "¡Formulario enviado correctamente! Redirigiendo..." is removed', () => {
	assert(
		!formsJsContent.includes('¡Formulario enviado correctamente! Redirigiendo...'),
...
test('Subtitle changed to "Redirigiendo a la página de confirmación..."', () => {
	assert(
		formsJsContent.includes('Redirigiendo a la página de confirmación...'),
...
test('"Gracias por completar el formulario" is removed from inline message', () => {
	...
	const successBlockMatch = formsJsContent.match(/if \( type === 'success' \) \{[\s\S]*?messageElement\.innerHTML = `[\s\S]*?`;/);
...
test('"Su respuesta ha sido registrada exitosamente" is removed from inline message', () => {
	...
	const successBlock = successBlockMatch[0];
	assert(
		!successBlock.includes('Su respuesta ha sido registrada exitosamente'),
...
test('Subtitle is hardcoded (not dynamic)', () => {
	...
	assert(
		/form-message__subtitle[^>]*>Redirigiendo a la página de confirmación\.\.\.</i.test(successBlock),
