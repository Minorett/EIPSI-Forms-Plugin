...
test( '4.5 - Button styling', () => contains( cssFile, '.eipsi-btn' ) );
...
test( '8.7 - CSS uses clinical research colors', () => {
	...
	return contains( content, '#005a87' ) && contains( content, '#e3f2fd' );
} );
...
test( '9.1 - Focus visible styles defined', () => {
	const content = readFile( 'assets/css/completion-message.css' );
	return (
		contains( content, ':focus-visible' ) && contains( content, 'outline:' )
	);
} );
