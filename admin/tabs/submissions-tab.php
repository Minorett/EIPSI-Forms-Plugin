...
<div class="eipsi-submissions-tab">
    ...
    if (isset($_GET['deleted']) && $_GET['deleted'] === '1') {
        ...
        <div class="notice notice-success is-dismissible">
            <p><?php _e('Response deleted successfully.', 'eipsi-forms'); ?></p>
        </div>
        <?php
    }
    ...
    <div class="eipsi-submissions-toolbar">
            ...
            <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                <form method="get" style="flex: 1; min-width: 300px;">
                    ...
                    <select name="form_filter" id="form_filter" onchange="this.form.submit()" style="padding: 8px; min-width: 200px;">
                        <option value=""><?php _e('All Forms', 'eipsi-forms'); ?></option>
                ...
                <button type="button" id="eipsi-sync-submissions" 
                        ...
                        title="<?php _e('Sync form list with database', 'eipsi-forms'); ?>">
                    🔄 <?php _e('Sync', 'eipsi-forms'); ?>
                </button>
            </div>
        </div>
        ...
        <div class="vas-export-buttons" style="margin: 20px 0;">
            ...
            <a href="<?php echo esc_url($excel_url); ?>" class="button button-primary">
                📊 <?php _e('Download Excel', 'eipsi-forms'); ?>
            </a>
        </div>
    </div>
    ...
    <table class="wp-list-table widefat fixed striped">
        <thead>
            <tr>
                ...
                <th style="width: 10%;"><?php _e('Form ID', 'eipsi-forms'); ?></th>
                <?php endif; ?>
                <th style="width: 12%;"><?php _e('Participant ID', 'eipsi-forms'); ?></th>
                <th style="width: 12%;"><?php _e('Date', 'eipsi-forms'); ?></th>
                <th style="width: 10%;"><?php _e('Time', 'eipsi-forms'); ?></th>
                <th style="width: 10%;">⏱️ <?php _e('Total Time', 'eipsi-forms'); ?></th>
                <th style="width: 10%;"><?php _e('Device', 'eipsi-forms'); ?></th>
                <th style="width: 12%;"><?php _e('Browser', 'eipsi-forms'); ?></th>
                <th style="width: 14%;"><?php _e('Actions', 'eipsi-forms'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php if (empty($results)): ?>
                <tr>
                    <td colspan="<?php echo $colspan; ?>" style="text-align: center; padding: 20px;">
                        <?php _e('No responses found.', 'eipsi-forms'); ?>
                    </td>
                </tr>
            <?php else: ?>
                <?php 
                        ...
                        <td>
                            <div class="vas-action-buttons">
                                <a href="#" class="button button-small vas-view-response" 
                                   ...
                                   title="<?php _e('View response details', 'eipsi-forms'); ?>">
                                    👁️
                                </a>
                                ...
                                <a href="<?php echo esc_url(wp_nonce_url(
                                   ...
                                   onclick="return confirm('<?php _e('Are you sure you want to delete this response?', 'eipsi-forms'); ?>')"
                                   title="<?php _e('Delete response', 'eipsi-forms'); ?>">
                                    🗑️
                                </a>
                            </div>
                        </td>
...
<div id="vas-response-modal" style="display:none; position: fixed; z-index: 10000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); overflow: hidden;">
    <div class="vas-modal-content" style="background-color: #fefefe; margin: 2% auto; padding: 20px; border: 1px solid #888; width: 90%; max-width: 800px; max-height: 85vh; border-radius: 5px; position: relative; display: flex; flex-direction: column;">
        <div style="flex-shrink: 0;">
            ...
            <h3 style="margin-right: 40px;"><?php _e('Session Metadata', 'eipsi-forms'); ?></h3>
            <p style="color: #666; font-size: 13px; margin: 5px 0 15px 0;"><?php _e('Complete responses with questionnaire answers are available via CSV/Excel export.', 'eipsi-forms'); ?></p>
        </div>
...
jQuery(document).ready(function($) {
    ...
    $('.vas-view-response').on('click', function(e) {
        ...
        $('#vas-modal-body').html('<div style="text-align: center; padding: 40px;"><p><?php _e('Loading response details...', 'eipsi-forms'); ?></p></div>');
        ...
        $.ajax({
                ...
                if (response.success) {
                    ...
                    $('#toggle-device-info').on('click', function() {
                        ...
                        if (section.is(':visible')) {
                            ...
                            $(this).html('🖥️ <?php _e('Show Device Fingerprint', 'eipsi-forms'); ?>');
                            $(this).css('background', '#6c757d');
                        } else {
                            section.slideDown('fast');
                            $(this).html('🖥️ <?php _e('Hide Device Fingerprint', 'eipsi-forms'); ?>');
                    ...
                    $('#toggle-timing-analysis').on('click', function() {
                        ...
                        if (section.is(':visible')) {
                            section.slideUp('fast');
                            $(this).html('⏱️ <?php _e('Show Timing Analysis', 'eipsi-forms'); ?>');
                            $(this).css('background', '#135e96');
                        } else {
                            section.slideDown('fast');
                            $(this).html('⏱️ <?php _e('Hide Timing Analysis', 'eipsi-forms'); ?>');
                    ...
                    $('#vas-modal-body').html(
                        ...
                        '<h3><?php _e('Error', 'eipsi-forms'); ?></h3>' +
                        '<p>' + (response.data || '<?php _e('Could not load response details', 'eipsi-forms'); ?>') + '</p>' +
                        '</div>'
                    );
                }
            },
            error: function(xhr, status, error) {
                $('#vas-modal-body').html(
                    '<div style="text-align: center; padding: 20px; color: #d63638;">' +
                    '<h3><?php _e('Connection Error', 'eipsi-forms'); ?></h3>' +
                    '<p><?php _e('Please try again or check the console for details.', 'eipsi-forms'); ?></p>' +
                    '<p><small><?php _e('Technical:', 'eipsi-forms'); ?> ' + error + '</small></p>' +
                    '</div>'
                );
    ...
    $('#eipsi-sync-submissions').on('click', function(e) {
        ...
        $button.html('⏳ <?php _e('Syncing...', 'eipsi-forms'); ?>').prop('disabled', true);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $button.html('✓ <?php _e('Updated!', 'eipsi-forms'); ?>').removeClass('button-secondary').addClass('button-primary');
                    ...
                    // Mostrar error
                    $button.html('❌ <?php _e('Error', 'eipsi-forms'); ?>');
                    setTimeout(function() {
            ...
            error: function(xhr, status, error) {
                ...
                $button.html('❌ <?php _e('Connection Error', 'eipsi-forms'); ?>');
                setTimeout(function() {
                    $button.html(originalText).removeClass('button-primary').addClass('button-secondary').prop('disabled', false);
                }, 2000);
                console.error('AJAX Error:', error);
