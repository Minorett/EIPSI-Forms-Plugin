...
<div class="eipsi-completion-tab">
    ...
    <div class="notice notice-info inline" style="margin: 0 0 20px 0;">
        <p>
            <strong><?php _e('Integrated Thank-You Page', 'vas-dinamico-forms'); ?></strong><br>
            <?php _e('After submitting a form, participants will see an integrated thank-you page on the same URL. Configure the title, message, logo display, and action button below.', 'vas-dinamico-forms'); ?>
        </p>
    </div>
    ...
    <form id="eipsi-completion-message-form" method="post">
        ...
        <div style="margin: 20px 0;">
            <label for="completion_title" style="display: block; margin-bottom: 8px; font-weight: 600;">
                <?php _e('Título', 'vas-dinamico-forms'); ?>
            </label>
            <input type="text" 
                   id="completion_title" 
                   name="title" 
                   value="<?php echo esc_attr($config['title']); ?>" 
                   placeholder="<?php echo esc_attr__('¡Gracias por completar el formulario!', 'vas-dinamico-forms'); ?>"
                   style="width: 100%; max-width: 600px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
            <small style="display: block; margin-top: 6px; color: #666;">
                <?php _e('Main heading shown on the thank-you page', 'vas-dinamico-forms'); ?>
            </small>
        </div>
        ...
        <div style="margin: 20px 0;">
            <label for="completion_message" style="display: block; margin-bottom: 8px; font-weight: 600;">
                <?php _e('Mensaje', 'vas-dinamico-forms'); ?>
            </label>
            <?php 
            wp_editor(
                $config['message'],
            ...
            ?>
            <small style="display: block; margin-top: 6px; color: #666;">
                <?php _e('Rich text content displayed to participants', 'vas-dinamico-forms'); ?>
            </small>
        </div>
        
        <!-- Options -->
        <div style="margin: 20px 0; background: #f8f9fa; padding: 15px; border-radius: 6px;">
            <h3><?php _e('Display Options', 'vas-dinamico-forms'); ?></h3>
            
            <label style="display: block; margin-bottom: 12px;">
                <input type="checkbox" name="show_logo" <?php checked($config['show_logo']); ?>>
                <?php _e('Mostrar logo del sitio', 'vas-dinamico-forms'); ?>
            ...
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #ddd;">
                <label style="display: block;">
                    ...
                    <?php _e('Animación sutil', 'vas-dinamico-forms'); ?>
                    <span style="color: #666; font-size: 0.9em; margin-left: 8px;">
                        (<?php _e('subtle confetti or fade effect', 'vas-dinamico-forms'); ?>)
                    </span>
                </label>
            </div>
        </div>
        ...
        <button type="submit" class="button button-primary" id="eipsi-save-completion">
            <?php _e('💾 Guardar Configuración', 'vas-dinamico-forms'); ?>
        </button>
    ...
    <div style="margin: 30px 0; padding: 20px; background: #f0f6fc; border-left: 4px solid #005a87; border-radius: 6px;">
        <h3 style="margin-top: 0;"><?php _e('💡 How it works', 'vas-dinamico-forms'); ?></h3>
        <ul style="margin: 10px 0;">
            <li><?php _e('After submitting any form, participants will see this thank-you page on the <strong>same URL</strong> (no external redirect)', 'vas-dinamico-forms'); ?></li>
            <li><?php _e('The page is dynamically generated and integrated into the form experience', 'vas-dinamico-forms'); ?></li>
            <li><?php _e('Perfect for kiosks, tablets, and clinical environments where URL consistency is important', 'vas-dinamico-forms'); ?></li>
        </ul>
    </div>
