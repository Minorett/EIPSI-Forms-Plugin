...
/**
 * Toggle follow-up reminders for a wave
 * 
 * @return void
 * @since 2.0.0
 */
function eipsi_toggle_follow_up_reminders_handler() {
    ...
    // Update wave
    $result = EIPSI_Wave_Service::update_wave($wave_id, array(
    ...
    wp_send_json_success(array(
        'message' => $enabled 
            ? __('Seguimiento automático activado', 'eipsi-forms')
            : __('Seguimiento automático desactivado', 'eipsi-forms')
    ));
}
add_action('wp_ajax_eipsi_toggle_follow_up_reminders', 'eipsi_toggle_follow_up_reminders_handler');

/**
 * Get reminder configuration for a wave
 * 
 * @return void
 * @since 2.0.0
 */
function eipsi_get_reminder_config_handler() {
    check_ajax_referer('eipsi_forms_nonce', 'nonce');
    ...
    // Get stored configuration from wave meta or options
    $config = get_post_meta($wave_id, '_eipsi_reminder_config', true);
    
    // If no custom config exists, return empty (frontend will use defaults)
    if (empty($config)) {
        wp_send_json_success(array(
            'config' => null,
            'using_defaults' => true
...
add_action('wp_ajax_eipsi_get_reminder_config', 'eipsi_get_reminder_config_handler');

/**
 * Save reminder configuration for a wave
 * 
 * @return void
 * @since 2.0.0
 */
function eipsi_save_reminder_config_handler() {
    check_ajax_referer('eipsi_forms_nonce', 'nonce');
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        wp_send_json_error(array('message' => __('Configuración inválida', 'eipsi-forms')));
    }
    
    // Save configuration as wave meta
    update_post_meta($wave_id, '_eipsi_reminder_config', $config);
    
    wp_send_json_success(array(
        'message' => __('Configuración guardada exitosamente', 'eipsi-forms')
    ));
}
