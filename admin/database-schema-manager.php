...
class EIPSI_Database_Schema_Manager {
    ...
    public static function repair_local_schema() {
        ...
        $longitudinal_pool_assignments_table = $wpdb->prefix . 'eipsi_pool_assignments';
        ...
        $repair_log = array(
            ...
            'pool_assignments_table' => array(
                'exists' => false,
                'created' => false,
                'columns_added' => array(),
            ),
            'pool_analytics_table' => array(
                'exists' => false,
                'created' => false,
                'columns_added' => array(),
            ),
        ...
        $email_confirmations_sync = self::sync_local_survey_email_confirmations_table();
        $repair_log['survey_email_confirmations_table']['exists'] = $email_confirmations_sync['exists'];
        $repair_log['survey_email_confirmations_table']['created'] = $email_confirmations_sync['created'];
        $repair_log['survey_email_confirmations_table']['columns_added'] = $email_confirmations_sync['columns_added'];
        ...
        $pool_assignments_sync = self::sync_local_pool_assignments_table();
        $repair_log['pool_assignments_table']['exists'] = $pool_assignments_sync['exists'];
        $repair_log['pool_assignments_table']['created'] = $pool_assignments_sync['created'];
        $repair_log['pool_assignments_table']['columns_added'] = $pool_assignments_sync['columns_added'];
        
        $pool_analytics_sync = self::sync_local_pool_analytics_table();
        $repair_log['pool_analytics_table']['exists'] = $pool_analytics_sync['exists'];
        $repair_log['pool_analytics_table']['created'] = $pool_analytics_sync['created'];
        $repair_log['pool_analytics_table']['columns_added'] = $pool_analytics_sync['columns_added'];
        ...
        $has_errors = false;
        foreach ( $repair_log as $key => $table_info ) {
        ...
        return $repair_log;
            ...
            if ( ! self::local_table_exists(  ) ) {
                ...
                dbDelta(  );
                ['randomization_assignments_table']['created'] = true;
                error_log( '[EIPSI Forms] Created randomization_assignments table' );
            }

            // Re-check core tables
            ['results_table']['exists'] = self::local_table_exists(  );
            ['events_table']['exists'] = self::local_table_exists(  );
            ['randomization_configs_table']['exists'] = self::local_table_exists(  );
            ['randomization_assignments_table']['exists'] = self::local_table_exists(  );

            if ( !  ) {
                ['results_table']['created'] = true;
                ['events_table']['created'] = true;
                error_log( '[EIPSI Forms] Schema repair: Core tables created' );
            }
        } else {
            ['results_table']['exists'] = true;
            ['events_table']['exists'] = true;
            ['randomization_configs_table']['exists'] = true;
            ['randomization_assignments_table']['exists'] = true;
        }

        // Create/sync longitudinal tables (always run to ensure they exist)
         = self::sync_local_survey_studies_table();
        ['survey_studies_table']['exists'] = ['exists'];
        ['survey_studies_table']['created'] = ['created'];
        ['survey_studies_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_participants_table();
        ['survey_participants_table']['exists'] = ['exists'];
        ['survey_participants_table']['created'] = ['created'];
        ['survey_participants_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_sessions_table();
        ['survey_sessions_table']['exists'] = ['exists'];
        ['survey_sessions_table']['created'] = ['created'];
        ['survey_sessions_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_waves_table();
        ['survey_waves_table']['exists'] = ['exists'];
        ['survey_waves_table']['created'] = ['created'];
        ['survey_waves_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_assignments_table();
        ['survey_assignments_table']['exists'] = ['exists'];
        ['survey_assignments_table']['created'] = ['created'];
        ['survey_assignments_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_magic_links_table();
        ['survey_magic_links_table']['exists'] = ['exists'];
        ['survey_magic_links_table']['created'] = ['created'];
        ['survey_magic_links_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_email_log_table();
        ['survey_email_log_table']['exists'] = ['exists'];
        ['survey_email_log_table']['created'] = ['created'];
        ['survey_email_log_table']['columns_added'] = ['columns_added'];

         = self::sync_local_survey_audit_log_table();
        ['survey_audit_log_table']['exists'] = ['exists'];
        ['survey_audit_log_table']['created'] = ['created'];
        ['survey_audit_log_table']['columns_added'] = ['columns_added'];
        ...
        ['survey_email_confirmations_table']['columns_added'] = ['columns_added'];
         ...
         = self::sync_local_longitudinal_pools_table();
        ['longitudinal_pools_table']['exists'] = ['exists'];
        ['longitudinal_pools_table']['created'] = ['created'];
        ['longitudinal_pools_table']['columns_added'] = ['columns_added'];

         = self::sync_local_pool_email_log_table();
        ['pool_email_log_table']['exists'] = ['exists'];
        ['pool_email_log_table']['created'] = ['created'];
        ['pool_email_log_table']['columns_added'] = ['columns_added'];

         = self::sync_local_pool_assignments_table();
        ['pool_assignments_table']['exists'] = ['exists'];
        ['pool_assignments_table']['created'] = ['created'];
        ['pool_assignments_table']['columns_added'] = ['columns_added'];
        
         = self::sync_local_pool_analytics_table();
        ['pool_analytics_table']['exists'] = ['exists'];
        ['pool_analytics_table']['created'] = ['created'];
        ['pool_analytics_table']['columns_added'] = ['columns_added'];

        // Phase 2 tables: participant access log + device data (v2.0.0+)
         = self::sync_local_survey_participant_access_log_table();
        ['survey_participant_access_log_table'] = array(
            'exists'        => ['exists'],
            'created'       => ['created'],
        ...
        if ( ! ['success'] ) {
         ...
         = self::sync_local_device_data_table();
        ['eipsi_device_data_table'] = array(
            'exists'        => ['exists'],
            'created'       => ['created'],
            'columns_added' => ['columns_added'],
        );
        ...
        if ( ['results_table']['exists'] ) {
             = self::repair_local_results_table(  );
            ['results_table']['columns_added'] = ;
        }

        // Repair events table
        if ( ['events_table']['exists'] ) {
             = self::repair_local_events_table(  );
            ['events_table']['columns_added'] = ;
        }

        // Repair randomization configs table
        if ( ['randomization_configs_table']['exists'] ) {
             = self::repair_local_randomization_configs_table(  );
            ['randomization_configs_table']['columns_added'] = ;
        }

        // Repair randomization assignments table
        if ( ['randomization_assignments_table']['exists'] ) {
             = self::repair_local_randomization_assignments_table(  );
            ['randomization_assignments_table']['columns_added'] = ;
        }
        ...
        // Calculate total columns added
         = 0;
        foreach (  as  ) {
            if ( isset( ['columns_added'] ) && is_array( ['columns_added'] ) ) {
                 += count( ['columns_added'] );
            }
        }

        if (  > 0 ) {
            error_log( '[EIPSI Forms] Schema repair completed - Columns added: ' . wp_json_encode(  ) );
        }
    ...
    private static function sync_local_longitudinal_pools_table() {
        ...
        $required_columns = array(
            ...
            'config' => "ALTER TABLE {$table_name} ADD COLUMN config JSON AFTER status",
            'page_id' => "ALTER TABLE {$table_name} ADD COLUMN page_id BIGINT(20) UNSIGNED DEFAULT NULL AFTER config",
            'created_at' => "ALTER TABLE {$table_name} ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP AFTER page_id",
            'updated_at' => "ALTER TABLE {$table_name} ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER created_at",
        );
    ...
    private static function get_table_creation_order() {
        return array(
            ...
            'eipsi_pool_assignments' => array(
                ...
                'sync_method' => 'sync_local_pool_assignments_table',
            ),
            'eipsi_pool_analytics' => array(
                'level' => 2,
                'dependencies' => array('eipsi_longitudinal_pools', 'survey_studies'),
                'sync_method' => 'sync_local_pool_analytics_table',
            ),
        );
    }
