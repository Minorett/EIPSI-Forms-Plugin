...
( function ( $ ) {
    ...
    function initWavesManager() {
        ...
        $( document ).on( 'change', '#has_time_limit', function () {
            $( '#time-limit-input-container' ).toggle( $( this ).is( ':checked' ) );
        } );
    ...
    function initParticipantsManagement() {
        ...
        // Delete Participant
        $( document ).on( 'click', '.eipsi-delete-participant-btn', function () {
            const participantId = $( this ).data( 'participant-id' );
            const hasSubmissions = $( this ).data( 'has-submissions' );

            let confirmMessage = eipsiWavesManagerData.strings.confirmDeleteParticipant ||
    ...
    function loadParticipants() {
        ...
        $.ajax( {
            url: eipsiWavesManagerData.ajaxUrl ||
                ( typeof ajaxurl !== 'undefined' ? ajaxurl : '/wp-admin/admin-ajax.php' ),
            type: 'GET',
            data: {
    ...
    function renderParticipantsTable( participants ) {
        ...
        participants.forEach( function ( p ) {
            ...
            const fullName = escapeHtml( ( p.first_name || '' ) + ' ' + ( p.last_name || '' ) ).trim() || '—';
            const registeredDate = p.created_at ? new Date( p.created_at ).toLocaleDateString() : '—';
            ...
            html +=
                ...
                '<td>' +
                '<button type="button" class="button button-small eipsi-edit-participant-btn" data-participant-id="' + p.id + '">Editar</button> ' +
                '<button type="button" class="button button-small button-link-delete eipsi-delete-participant-btn" data-participant-id="' + p.id + '">Eliminar</button>' +
                '</td>' +
                '</tr>';
        } );
        $tbody.html( html );
    ...
    function loadParticipantData( participantId ) {
        const $btn = $( '.eipsi-edit-participant-btn[data-participant-id="' + participantId + '"]' );
        const originalText = $btn.text();
        $btn.text( 'Cargando...' ).prop( 'disabled', true );
    ...
    function populateParticipantForm( participantData ) {
        ...
        $( '#participant_is_active' ).prop( 'checked', parseInt( participantData.is_active ) === 1 );
    }
    ...
    function saveParticipant() {
        ...
        if ( ! isEdit ) {
            ...
            formData.is_active = $( '#participant_is_active' ).is( ':checked' ) ? 1 : 0;
        }
        ...
        $.ajax( {
            ...
            success( response ) {
                if ( response.success ) {
                    ...
                    showNotification( response.data || 'Error al guardar participante', 'error' );
                }
            },
            error() {
                showNotification( 'Error de conexión', 'error' );
    ...
    function deleteParticipant( participantId, deleteData ) {
        const $btn = $( '.eipsi-delete-participant-btn[data-participant-id="' + participantId + '"]' );
        const originalText = $btn.text();
        $btn.text( 'Eliminando...' ).prop( 'disabled', true );
        ...
        $.ajax( {
            ...
            success( response ) {
                if ( response.success ) {
                    showNotification(
                        eipsiWavesManagerData.strings.participantDeleted || 'Participante eliminado',
                        'success'
                    );
                    loadParticipants(); // Refresh list
                } else {
                    showNotification( response.data || 'Error al eliminar participante', 'error' );
                }
            },
            error() {
                showNotification( 'Error de conexión', 'error' );
