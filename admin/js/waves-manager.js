...
( function ( $ ) {
    ...
    function initWavesManager() {
        ...
        $( document ).on( 'change', '#start_date, #due_date, #wave_index', function () {
...
document ).on( 'click', '.eipsi-send-reminder-btn', function () {
st waveId = $( this ).data( 'wave-id' );
EmailPreviewModal( waveId, 'reminder' );
);

document ).on( 'click', '.eipsi-send-manual-reminder-btn', function () {
st waveId = $( this ).data( 'wave-id' );
EmailPreviewModal( waveId, 'manual' );
);
...
{
eipsiWavesManagerData.ajaxUrl || ajaxurl,
pe: 'GET',
{
: 'eipsi_get_pending_participants',
once: eipsiWavesManagerData.wavesNonce,
_id: eipsiWavesManagerData.studyId,
emailPreviewWaveId,
response ) {
( response.success && response.data ) {
st $select = $( '#email-preview-participant' );
d( 'option:not(:first)' ).remove();
se.data.forEach( function ( p ) {
st fullName = ( p.first_name || '' ) + ' ' + ( p.last_name || '' );
d( '<option value="' + p.id + '">' + escapeHtml( fullName.trim() || p.email ) + ' (' + escapeHtml( p.email ) + ')</option>' );
);
);
}

function loadEmailPreview() {
st waveId = emailPreviewWaveId;
st emailType = $( '#email-preview-type' ).val();
st participantId = $( '#email-preview-participant' ).val();
...
response ) {
( response.success && response.data ) {
st data = response.data;
'#email-preview-subject' ).text( data.subject || '' );
'#email-preview-body' ).html( data.content || '' );
( data.is_sample ) {
'#email-preview-body' ).prepend( '<div style="background: #e8f4f8; padding: 8px 12px; margin-bottom: 15px; border-radius: 4px; font-size: 0.85em; color: #0066cc;">📝 <strong>Vista previa de muestra</strong> - Los datos del participante se sustituirán al enviar.</div>' );
else {
'#email-preview-body' ).html( '<div style="text-align: center; padding: 40px; color: #d63638;">Error al cargar la vista previa</div>' );
{
'#email-preview-body' ).html( '<div style="text-align: center; padding: 40px; color: #d63638;">Error de conexión</div>' );
);
}
...
st studyId = eipsiWavesManagerData.studyId;
st waveId = $( '#wave_id' ).val();
st waveIndex = $( '#wave_index' ).val();
st startDate = $( '#start_date' ).val();
st dueDate = $( '#due_date' ).val();
...
response ) {
( response.success && response.data ) {
st validation = response.data;
( validation.errors && validation.errors.length > 0 ) {
otification( validation.errors[ 0 ], 'error' );
( validation.warnings && validation.warnings.length > 0 ) {
dingValidationWarnings = validation.warnings;
else {
dingValidationWarnings = null;
);
}
...
( pendingValidationWarnings && pendingValidationWarnings.length > 0 ) {
WarningModal( pendingValidationWarnings );
 true;
 false;
}

function showValidationWarningModal( warnings ) {
st $list = $( '#validation-warnings-list' );
'<div class="notice notice-warning" style="margin: 0;"><p><strong>⚠️ Se detectaron las siguientes advertencias:</strong></p><ul style="margin: 10px 0;">' + warnings.map( function ( w ) { return '<li>' + escapeHtml( w ) + '</li>'; } ).join( '' ) + '</ul><p>¿Deseas guardar de todos modos o revisar las fechas?</p></div>' );
