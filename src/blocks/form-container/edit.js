...
export default function Edit({
      ...
      ? {
          ...
          probabilities:
            ...
            typeof randomConfigAttr.probabilities === "object"
              ? randomConfigAttr.probabilities
              : {},
          manualAssigns: Array.isArray(randomConfigAttr.manualAssigns)
  ...
  const loadAvailableForms = async () => {
    ...
    try {
      ...
      if (data?.success) {
        setAvailableForms(Array.isArray(data.data) ? data.data : []);
      }
    } catch (error) {
  ...
  const addFormToRandom = (formIdToAdd) => {
    ...
    const currentRandomConfig =
      attributes.randomConfig && typeof attributes.randomConfig === "object"
        ? {
            ...RANDOMIZATION_DEFAULT_CONFIG,
    ...
    const newForms = [...currentRandomConfig.forms, parseInt(formIdToAdd)];
      ...
      attributes.randomConfig && typeof attributes.randomConfig === "object"
        ? {
            ...
            probabilities:
              ...
              typeof attributes.randomConfig.probabilities === "object"
                ? attributes.randomConfig.probabilities
                : {},
          }
        : { ...RANDOMIZATION_DEFAULT_CONFIG };
  ...
  const removeManualAssign = (index) => {
    const newAssigns = randomConfig.manualAssigns.filter((_, i) => i !== index);
    updateRandomConfig("manualAssigns", newAssigns);
  };
  ...
  const generateRandomLink = async () => {
    ...
    try {
      ...
      // eslint-disable-next-line no-alert
      window.alert(
        `${__("Link generado (fallback):", "eipsi-forms")}\n${fallbackLink}`,
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (typeof useCustomCompletion !== "boolean") {
      const hasCustomCompletionOverride =
        ...
        (completionButtonLabel &&
          completionButtonLabel !== COMPLETION_DEFAULTS.buttonLabel) ||
        !!completionLogoUrl;
  ...
  // Cargar templates disponibles por AJAX
  useEffect(() => {
  ...
  const loadAvailableTemplates = async () => {
    ...
    try {
      ...
      const response = await fetch(
        ajaxUrl +
          "?action=eipsi_get_demo_templates&nonce=" +
          encodeURIComponent(nonce),
        { credentials: "same-origin" },
      );
      ...
      const data = await response.json();
      if (data?.success) {
        setAvailableTemplates(Array.isArray(data.data) ? data.data : []);
      }
    } catch (error) {
      // eslint-disable-next-line no-console
  ...
  const templateOptions = [
    {
      label: __("Elegí una plantilla", "eipsi-forms"),
      value: "",
    },
  ...
  const handleApplyTemplate = () => {
    ...
    try {
      ...
      allowedAttrKeys.forEach((key) => {
        if (Object.prototype.hasOwnProperty.call(containerAttrs, key)) {
      ...
      setTemplateNotice({
        status: "success",
        message: __("Plantilla aplicada correctamente.", "eipsi-forms"),
      });
    } catch (error) {
      // eslint-disable-next-line no-console
      ...
      <InspectorControls>
        ...
        >
          ...
          {loadingTemplates && (
            <Notice status="info" isDismissible={false}>
              {__("Cargando plantillas…", "eipsi-forms")}
            </Notice>
          )}

          {availableTemplates.length === 0 && !loadingTemplates && (
            <Notice status="info" isDismissible={false}>
              {__("No hay plantillas disponibles.", "eipsi-forms")}
            </Notice>
          )}
          ...
          {availableTemplates.length > 0 && (
            <>
              ...
              <SelectControl
                label={__("Plantillas EIPSI (demo)", "eipsi-forms")}
              ...
              {selectedTemplate && (
                ...
                >
                  {__("Aplicar plantilla", "eipsi-forms")}
                </Button>
              )}
            </>
          )}
        ...
        >
          ...
          <TextControl
            ...
            onChange={(value) => setAttributes({ submitButtonLabel: value })}
          />
          <TextareaControl
        ...
        >
          <ToggleControl
            label={__('Mostrar botón "Anterior"', "eipsi-forms")}
          ...
          />
          <ToggleControl
            label={__("Mostrar barra de progreso", "eipsi-forms")}
        ...
        >
          <ToggleControl
            label={__("Personalizar página de finalización", "eipsi-forms")}
          ...
          {customCompletionEnabled && (
            <>
              <TextControl
                label={__("Título de finalización", "eipsi-forms")}
                value={completionTitle}
                onChange={(value) => setAttributes({ completionTitle: value })}
                help={__(
              ...
              />
              <TextareaControl
                label={__("Mensaje de finalización", "eipsi-forms")}
              ...
              <MediaUploadCheck>
                <div style={{ marginTop: "16px" }}>
                  ...
                  >
                    {__("Logo o imagen (opcional)", "eipsi-forms")}
                  </p>
                  {completionLogoUrl && (
                    <div style={{ marginBottom: "12px" }}>
                      <img
                        src={completionLogoUrl}
                        alt={__("Logo del consultorio", "eipsi-forms")}
                        style={{
                          maxWidth: "200px",
                          height: "auto",
                          borderRadius: "8px",
                    ...
                    render={({ open }) => (
                      <div>
                        <Button variant="secondary" onClick={open}>
                          {completionLogoUrl
                            ? __("Cambiar imagen", "eipsi-forms")
                            : __("Seleccionar imagen", "eipsi-forms")}
                        </Button>
                        {completionLogoUrl && (
                          <Button
                            variant="tertiary"
        ...
        >
          ...
          <Button variant="secondary" onClick={() => setIsMapOpen(true)}>
        ...
        >
          <FormStylePanel
            ...
            setPresetName={(name) => setAttributes({ presetName: name })}
          />
        </PanelBody>
        ...
        >
          <ToggleControl
            label={__("Capturar tiempos por página", "eipsi-forms")}
            checked={capturePageTiming}
            onChange={(value) => setAttributes({ capturePageTiming: value })}
          />
          <ToggleControl
            label={__("Capturar tiempos por campo", "eipsi-forms")}
            checked={captureFieldTiming}
            onChange={(value) => setAttributes({ captureFieldTiming: value })}
          />
          <hr />
          <ToggleControl
            label={__("⏱️ Hide Timing Analysis", "eipsi-forms")}
            checked={!showTimingAnalysis}
            onChange={(value) => setAttributes({ showTimingAnalysis: !value })}
            help={__(
        ...
        >
          <ToggleControl
            label={__("Activar aleatorización de asignación", "eipsi-forms")}
            checked={!!useRandomization}
            onChange={(value) => setAttributes({ useRandomization: !!value })}
            help={__(
          ...
          {useRandomization && (
            <div className="eipsi-randomization-panel">
              ...
              <BaseControl
                ...
                label={__("Formularios para aleatorizar", "eipsi-forms")}
              >
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <SelectControl
                    ...
                    options={[
                      {
                        label: __("Seleccionar formulario…", "eipsi-forms"),
                        value: "",
                      },
                ...
                {randomConfig.forms.length > 0 && (
                  <div className="eipsi-forms-list">
                    {randomConfig.forms.map((selectedFormId) => (
                      <div
                        key={selectedFormId}
                      ...
                      >
                        ...
                        >
                          <strong>{getFormName(selectedFormId)}</strong>
                          <Button
                            variant="tertiary"
                            isDestructive
                            onClick={() => removeFormFromRandom(selectedFormId)}
                            icon="no-alt"
                          />
                        </div>
                        <div
                        ...
                        >
                          <input
                            ...
                            max="100"
                            value={
                              randomConfig.probabilities[selectedFormId] || 0
                            }
                            onChange={(e) =>
                              updateProbability(selectedFormId, e.target.value)
                            }
                            style={{
                              flex: 1,
                            }}
                    ...
                    <div
                      style={{
                        ...
                        padding: "8px",
                        background:
                          totalProbability === 100 ? "#dcfce7" : "#fef3c7",
                        borderRadius: "6px",
                        fontWeight: 600,
                        color: totalProbability === 100 ? "#166534" : "#92400e",
                      }}
                    >
                      {__("Total:", "eipsi-forms")} {totalProbability}%
                    </div>
                  </div>
                )}
              </BaseControl>
              ...
              <SelectControl
                ...
                options={[
                  {
                    label: __("Simple (uniforme)", "eipsi-forms"),
                    value: "simple",
                  },
                  {
                    label: __("Con seed reproducible", "eipsi-forms"),
                    value: "seeded",
                  },
                ]}
                onChange={(value) => updateRandomConfig("method", value)}
                help={__(
              ...
              >
                ...
                >
                  <TextControl
                    placeholder={__("Email del participante", "eipsi-forms")}
                    value={manualEmail}
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={!manualEmail || !manualFormId}
                    >
                ...
                {randomConfig.manualAssigns.length > 0 && (
                  ...
                  >
                    ...
                    <tbody>
                      {randomConfig.manualAssigns.map((assign, index) => (
                        <tr key={index}>
                          <td
                            style={{
                              padding: "8px",
                          ...
                          >
                            <Button
                              variant="tertiary"
                              isDestructive
                              onClick={() => removeManualAssign(index)}
                              icon="no-alt"
                            />
                          </td>
                        </tr>
              ...
              >
                {linkCopied
                  ...
                  : "🔗 " + __("Generar link con random", "eipsi-forms")}
              </Button>
              ...
              {randomConfig.forms.length >= 2 && (
                ...
                >
                  <strong>{__("Vista previa:", "eipsi-forms")}</strong>
                  <br />
                  {__("Aleatorización activa:", "eipsi-forms")}{" "}
                  {randomConfig.forms
                    .map(
                      (id) =>
                        `${getFormName(id)} (${
                          randomConfig.probabilities[id] || 0
                        }%)`,
                    )
                    .join(" | ")}
                  <br />
                  ...
                  {randomConfig.method === "seeded"
                    ? __("Con seed reproducible", "eipsi-forms")
                    : __("Simple (uniforme)", "eipsi-forms")}
                  <br />
                  {__("Overrides manuales:", "eipsi-forms")}{" "}
            ...
            <div className="eipsi-form-placeholder">
              <div className="components-placeholder">
                <div className="components-placeholder__label">
                  {__("EIPSI Form Container", "eipsi-forms")}
                </div>
          ...
          {formId && (
            <div className="eipsi-form-preview-wrapper">
              ...
              <div className="form-footer">
                <button type="button" className="eipsi-submit-button" disabled>
                  {submitButtonLabel || __("Submit", "eipsi-forms")}
                </button>
              </div>
            </div>
          )}
