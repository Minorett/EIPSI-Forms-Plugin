...
function cleanFunctions( content, functionsToRemove ) {
	...
	functionsToRemove.forEach( ( funcName ) => {
		...
		const patterns = [
			...
			new RegExp(
				`const\\s+${ funcName }\\s*=\\s*function[\\s\\S]*?};\\s*\\n?`,
				'g'
			),
			// function nombre
			new RegExp(
			...
			// const nombre = () =>
			new RegExp(
				`const\\s+${ funcName }\\s*=\\s*\\([^)]*\\)\\s*=>\\s*\\{[\\s\\S]*?\\}\\s*\\n?`,
				'g'
			),
		];
...
function addImports( content ) {
	...
	if (
		! cleaned.includes( "from '../../utils/helpers'" ) &&
		! cleaned.includes( "from '../utils/helpers'" )
	) {
		const utilsImport =
			"import { renderHelperText, getFieldId, calculateMaxValue, renderDescriptionBody, renderConsentBody } from '../../utils/helpers';";
