...
function eipsi_shortcode_help_metabox() {
    ...
    foreach ($screens as $screen) {
        add_meta_box(
            ...
            __('Shortcode de Formularios EIPSI', 'vas-dinamico-forms'),
...
function eipsi_shortcode_help_callback($post) {
    ...
    <div class="eipsi-shortcode-help">
        <p style="font-size: 13px; margin: 0 0 12px;">
            <?php esc_html_e('Insertá formularios usando este formato:', 'vas-dinamico-forms'); ?>
        </p>
        ...
        if (!empty($templates)) {
            ...
            <p style="font-size: 13px; margin: 12px 0 8px; font-weight: 600;">
                <?php esc_html_e('Formularios disponibles:', 'vas-dinamico-forms'); ?>
            </p>
                ...
                <?php foreach ($templates as $template) : ?>
                    <div style="padding: 8px; border-bottom: 1px solid #f0f0f1; font-size: 12px;">
                        <div style="font-weight: 600; margin-bottom: 4px;">
                            <?php echo esc_html($template->post_title ?: __('(Sin título)', 'vas-dinamico-forms')); ?>
                        </div>
                        <code style="background: #f0f0f1; padding: 2px 6px; border-radius: 2px; font-size: 11px; cursor: pointer;" 
                              ...
                              title="<?php esc_attr_e('Clic para copiar', 'vas-dinamico-forms'); ?>">
            ...
            <p style="font-size: 11px; color: #666; margin: 8px 0 0;">
                <em><?php esc_html_e('💡 Clic en un shortcode para copiarlo', 'vas-dinamico-forms'); ?></em>
            </p>
            <?php
        } else {
            ?>
            <p style="font-size: 13px; margin: 12px 0 0; padding: 8px; background: #f0f0f1; border-radius: 3px;">
                <?php esc_html_e('No hay formularios creados aún.', 'vas-dinamico-forms'); ?>
                <br>
                <a href="<?php echo esc_url(admin_url('post-new.php?post_type=eipsi_form_template')); ?>" style="font-size: 13px;">
                    <?php esc_html_e('+ Crear tu primer formulario', 'vas-dinamico-forms'); ?>
                </a>
            </p>
            <?php
        }
...
function eipsi_add_shortcode_indicator_column($columns) {
    $columns['eipsi_forms'] = '<span class="dashicons dashicons-feedback" title="' . esc_attr__('Formularios EIPSI', 'vas-dinamico-forms') . '"></span>';
    return $columns;
}
...
function eipsi_shortcode_indicator_column_content($column, $post_id) {
    if ($column === 'eipsi_forms') {
        ...
        $has_block = has_block('vas-dinamico/form-block', $content) || 
                     has_block('vas-dinamico/form-container', $content);
        
        if ($has_shortcode || $has_block) {
            echo '<span class="dashicons dashicons-yes-alt" style="color: #00a32a;" title="' . esc_attr__('Contiene formularios EIPSI', 'vas-dinamico-forms') . '"></span>';
        } else {
