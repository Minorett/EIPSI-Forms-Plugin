...
function test( name, fn ) {
	...
	try {
		...
		if ( result === true ) {
			...
			console.log(
				`${ colors.yellow }⚠${ colors.reset } ${ name }: ${ result.message }`
			);
		} else {
			results.tests[ name ] = { status: 'FAIL', message: result.message };
			results.summary.failed++;
			console.log(
				`${ colors.red }✗${ colors.reset } ${ name }: ${ result.message }`
			);
		}
	} catch ( error ) {
		results.tests[ name ] = {
		...
		results.summary.failed++;
		console.log(
			`${ colors.red }✗${ colors.reset } ${ name }: ${ error.message }`
		);
	}
}
...
// ============================================================================
console.log(
	`${ colors.blue }[1/6] Build Artifact Integrity${ colors.reset }`
);
...
test( 'Build JS bundle size within acceptable limits (< 150KB)', () => {
	...
	if ( size > 150 * 1024 ) {
		return {
			warning: true,
			message: `Bundle size ${ ( size / 1024 ).toFixed(
				2
			) }KB exceeds 150KB threshold`,
		};
	}
	return true;
} );
...
test( 'Frontend JS bundle size within acceptable limits (< 100KB)', () => {
	...
	if ( size > 100 * 1024 ) {
		return {
			warning: true,
			message: `Frontend JS ${ ( size / 1024 ).toFixed(
				2
			) }KB exceeds 100KB threshold`,
		};
	}
	return true;
} );
...
test( 'Total CSS size within acceptable limits (< 100KB)', () => {
	...
	if ( totalCSS > 100 * 1024 ) {
		return {
			warning: true,
			message: `Total CSS ${ ( totalCSS / 1024 ).toFixed(
				2
			) }KB exceeds 100KB threshold`,
		};
	}
...
test( 'Estimate parse time for main JS bundle', () => {
	...
	if ( estimatedParseTime > 100 ) {
		return {
			warning: true,
			message: `Estimated parse time ${ estimatedParseTime.toFixed(
				2
			) }ms may impact performance`,
		};
	}
	return true;
} );
...
test( 'Estimate network transfer time (3G)', () => {
	...
	if ( transferTime3G > 3000 ) {
		return {
			warning: true,
			message: `3G transfer time ${ transferTime3G.toFixed(
				2
			) }ms exceeds 3 second threshold`,
		};
	}
	return true;
} );
...
test( 'No blocking resources identified', () => {
	...
	if (
		mainFile.includes( "wp_enqueue_script('" ) &&
		! mainFile.includes( 'defer' )
	) {
		return {
			warning: true,
...
test( 'Memory footprint estimation', () => {
	...
	if ( estimatedMemory > 10 ) {
		return {
			warning: true,
			message: `Estimated memory ${ estimatedMemory.toFixed(
				2
			) }MB may be high for mobile devices`,
		};
	}
	return true;
} );
...
console.log( `\nBundle Sizes:` );
console.log(
	`  Build Output:    ${ ( totalBuildSize / 1024 ).toFixed( 2 ) } KB`
);
console.log(
	`  Frontend Assets: ${ ( totalFrontendSize / 1024 ).toFixed( 2 ) } KB`
);
...
console.log(
	`  ${ colors.red }Failed:${ colors.reset }   ${ results.summary.failed }`
);
console.log(
	`  ${ colors.yellow }Warnings:${ colors.reset } ${ results.summary.warnings }`
);
