...
testCase('allowBackwardsNav attribute is defined', 
    blockConfig.attributes && blockConfig.attributes.allowBackwardsNav !== undefined);

testCase('allowBackwardsNav type is boolean', 
    blockConfig.attributes?.allowBackwardsNav?.type === 'boolean');

testCase('allowBackwardsNav default is true (ON)', 
...
testCase('edit.js destructures allowBackwardsNav from attributes',
    fileContains(editJsPath, 'allowBackwardsNav') && 
    editJs.includes('const {') && 
    editJs.includes('allowBackwardsNav'));

testCase('ToggleControl for allowBackwardsNav exists',
    editJs.includes('ToggleControl') && 
    editJs.includes('Allow backwards navigation'));

testCase('ToggleControl is checked based on allowBackwardsNav',
    editJs.includes('checked={ !! allowBackwardsNav }'));

testCase('ToggleControl updates allowBackwardsNav attribute',
    editJs.includes('setAttributes( { allowBackwardsNav: !! value } )'));

testCase('Toggle help text explains functionality',
...
testCase('save.js destructures allowBackwardsNav from attributes',
    saveJs.includes('allowBackwardsNav'));

testCase('save.js saves allowBackwardsNav as data attribute',
    saveJs.includes('data-allow-backwards-nav') || 
    saveJs.includes('allowBackwardsNav'));

testCase('save.js converts boolean to string correctly',
    saveJs.includes('allowBackwardsNav ? \'true\' : \'false\'') ||
    saveJs.includes('data-allow-backwards-nav={') && saveJs.includes('allowBackwardsNav'));
...
testCase('Frontend reads data-allow-backwards-nav attribute',
...
testCase('Frontend parses allowBackwardsNav value correctly',
    frontendJs.includes('rawAllowBackwards') || 
    frontendJs.includes('allowBackwardsNav'));

testCase('Frontend uses allowBackwardsNav to control prev button visibility',
    frontendJs.includes('shouldShowPrev') && 
    frontendJs.includes('allowBackwardsNav'));

testCase('Default behavior is TRUE (back button enabled) when attribute missing',
    frontendJs.match(/\?\s*false\s*:\s*true/) !== null &&
    (frontendJs.includes('=== \'false\'') || frontendJs.includes('!== \'false\'')));
...
testCase('allowBackwardsNav logic is present in updatePaginationDisplay',
    allowBackwardsNavLogicMatch !== null);

if (allowBackwardsNavLogicMatch) {
    const logic = allowBackwardsNavLogicMatch[0];
    
    testCase('Logic uses explicit true/false conversion',
        logic.match(/\?\s*false\s*:\s*true/) !== null);
    
    testCase('Logic checks for \'false\' string',
        logic.includes('=== \'false\'') || logic.includes('!== \'false\''));
    
    testCase('Logic checks for \'0\' string',
        logic.includes('=== \'0\'') || logic.includes('!== \'0\''));
    
    testCase('Logic does NOT check for empty string as disabled',
        !logic.includes('!== \'\''));
}

testCase('shouldShowPrev checks allowBackwardsNav',
    frontendJs.includes('shouldShowPrev') && 
    frontendJs.includes('allowBackwardsNav'));

testCase('shouldShowPrev checks hasHistory',
    frontendJs.includes('shouldShowPrev') && 
    frontendJs.includes('hasHistory'));

testCase('prevButton visibility is controlled by shouldShowPrev',
...
testCase('Navigation Settings panel exists in edit.js',
    editJs.includes('Navigation Settings'));

testCase('Navigation Settings panel is initially closed',
    editJs.includes('Navigation Settings') && 
    editJs.match(/Navigation Settings[\s\S]*?initialOpen\s*=\s*\{\s*false\s*\}/));

testCase('Form saves data-allow-backwards-nav to HTML',
    saveJs.includes('data-allow-backwards-nav'));

testCase('Frontend JavaScript checks dataset property',
    frontendJs.includes('form.dataset.allowBackwardsNav'));
...
const defaultHandlingCorrect = 
    (frontendJs.match(/rawAllowBackwards\s*===\s*'false'\s*\|\|\s*rawAllowBackwards/) !== null ||
     frontendJs.match(/rawAllowBackwards\s*===\s*'false'/) !== null) &&
    frontendJs.match(/\?\s*false\s*:\s*true/) !== null;

testCase('Frontend defaults to TRUE (enabled) for undefined/null/empty values',
    defaultHandlingCorrect);

testCase('block.json default matches frontend default (both TRUE)',
    blockConfig.attributes?.allowBackwardsNav?.default === true);
