...
class CoreInteractivityTester {
	...
	testLikertFields() {
		...
		this.recordTest(
			'Likert',
			'initLikertFields function exists',
			hasInitFunction
		);

		// Test 2: Check for keyboard navigation support
		const hasKeyboardSupport =
			this.jsCode.includes( 'ArrowLeft' ) ||
			this.jsCode.includes( 'ArrowRight' );
		this.recordTest(
			'Likert',
		...
		const hasLikertStyles =
			this.cssCode.includes( '.likert-option' ) ||
			this.cssCode.includes( '.eipsi-likert-field' );
		this.recordTest(
			'Likert',
			'CSS styles for likert fields',
			hasLikertStyles
		);
	...
	testVasSlider() {
		...
		this.recordTest(
			'VAS Slider',
			'initVasSliders function exists',
			hasInitFunction
		);
		...
		const hasAllKeys = keyboardKeys.every( ( key ) =>
			this.jsCode.includes( key )
		);
		this.recordTest(
			'VAS Slider',
		...
		const hasTouchSupport =
			this.jsCode.includes( 'pointerdown' ) ||
			this.jsCode.includes( 'touchstart' );
		this.recordTest(
			'VAS Slider',
		...
		const hasMouseSupport = this.jsCode.match(
			/addEventListener.*['"]input['"]/
		);
		this.recordTest(
			'VAS Slider',
			'Mouse drag support (input event)',
			!! hasMouseSupport
		);

		// Test 5: Check for value readout updates
		const hasValueUpdate =
			this.jsCode.includes( 'aria-valuenow' ) ||
		...
		const hasThrottling =
			this.jsCode.includes( 'throttled' ) ||
		...
		const hasLabels =
			this.cssCode.includes( '.vas-slider-label' ) ||
		...
		const hasSliderStyles =
			this.cssCode.match( /input\[type=['"]range['"]\]/ ) ||
	...
	testRadioInputs() {
		...
		const hasRadioInit =
			this.jsCode.includes( 'initRadioFields' ) ||
		...
		const hasChangeHandler = this.jsCode.match(
			/radio.*addEventListener.*change/s
		);
		this.recordTest(
			'Radio',
		...
		const hasCheckedStyles =
			this.cssCode.match( /radio.*:checked/i ) ||
	...
	testTextInputs() {
		...
		const hasValidation =
			this.jsCode.includes( 'validateField' ) &&
		...
		const hasBlurValidation =
			this.jsCode.includes( 'validateOnBlur' ) ||
		...
		const hasLabelFor =
			this.cssCode.includes( 'label[for]' ) ||
		...
		const hasErrorDisplay =
			this.cssCode.includes( '.has-error' ) ||
		...
		const hasFocusStyles =
			this.cssCode.match( /input.*:focus/i ) ||
	...
	testInteractiveStates() {
		...
		const mobileFocusPattern =
			/@media.*max-width.*outline-width:\s*(\d+)px/s;
		const mobileFocusMatch = this.cssCode.match( mobileFocusPattern );
		const hasMobileFocus =
			mobileFocusMatch && parseInt( mobileFocusMatch[ 1 ] ) >= 3;
		this.recordTest(
			'Interactive States',
			'Mobile focus enhancement (≥3px)',
			hasMobileFocus,
			! hasMobileFocus
				? 'Mobile focus should be thicker than desktop'
				: null
		);
		...
		const disabledCount = ( this.cssCode.match( /:disabled/g ) || [] )
			.length;
		this.recordTest(
			'Interactive States',
		...
		const cssVarCount = ( this.cssCode.match( /var\(--eipsi-/g ) || [] )
			.length;
		this.recordTest(
			'Interactive States',
			'Design token usage (CSS vars)',
			cssVarCount > 50
		);

		// Test 7: Transition definitions
		const hasTransitions =
			this.cssCode.includes( 'transition' ) &&
		...
		const hasPrimaryColor = this.cssCode.includes(
		...
		const hasTouchTargets =
			this.cssCode.match( /min-(width|height):\s*44px/ ) ||
	...
	testJavaScriptIntegration() {
		...
		const hasTryCatch =
			( this.jsCode.match( /try\s*{/g ) || [] ).length > 0;
		this.recordTest(
			'JS Integration',
		...
		const hasConsoleWarn =
			this.jsCode.includes( 'console.warn' ) ||
		...
		const hasEventDelegation =
			this.jsCode.includes( 'querySelectorAll' ) &&
		...
		const hasSubmitHandler =
			this.jsCode.includes( 'handleSubmit' ) &&
		...
		const hasValidationFramework =
			this.jsCode.includes( 'validateField' ) &&
			this.jsCode.includes( 'has-error' );
		this.recordTest(
			'JS Integration',
			'Validation framework',
			hasValidationFramework
		);
	...
	generateMarkdownReport() {
		...
		report += `  - \`assets/js/eipsi-forms.js\` (${
			this.jsCode.split( '\n' ).length
		} lines)\n`;
		report += `  - \`assets/css/eipsi-forms.css\` (${
			this.cssCode.split( '\n' ).length
		} lines)\n\n`;
	...
	analyzeLikert() {
		let analysis = `**Status:** `;
		const likertTests = this.results.tests.filter(
			( t ) => t.category === 'Likert'
		);
	...
	analyzeVasSlider() {
		let analysis = `**Status:** `;
		const vasTests = this.results.tests.filter(
			( t ) => t.category === 'VAS Slider'
		);
	...
	analyzeRadioInputs() {
		let analysis = `**Status:** `;
		const radioTests = this.results.tests.filter(
			( t ) => t.category === 'Radio'
		);
	...
	analyzeTextInputs() {
		let analysis = `**Status:** `;
		const textTests = this.results.tests.filter(
			( t ) => t.category === 'Text Input'
		);
	...
	analyzeInteractiveStates() {
		let analysis = `**Status:** `;
		const stateTests = this.results.tests.filter(
			( t ) => t.category === 'Interactive States'
		);
	...
	run() {
		...
		console.log(
			`║   EIPSI Forms - Core Interactivity Test Suite v1.0           ║`
		);
		console.log(
			`║   Testing: Likert, VAS, Radio, Text Inputs & States          ║`
		);
		console.log(
			`╚═══════════════════════════════════════════════════════════════╝`
		);
