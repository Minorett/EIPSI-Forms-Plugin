...
if (/@media\s*\(max-width:\s*374px\)/i.test(cssContent)) {
    ...
    testFail('320px breakpoint missing', 'Should have @media (max-width: 374px)');
}

// Test 2: vas-dinamico-form padding
const breakpoint374Match = cssContent.match(/@media\s*\(max-width:\s*374px\)\s*\{([\s\S]*?)(?=@media|\/\*\s*===|$)/i);
if (breakpoint374Match) {
    const breakpoint374Content = breakpoint374Match[1];
    
    if (/\.vas-dinamico-form[\s\S]*?padding:\s*0\.75rem/i.test(breakpoint374Content)) {
    ...
    if (/\.vas-value-number[\s\S]*?font-size:\s*1\.5rem/i.test(breakpoint374Content)) {
    ...
    if (/\.likert-item[\s\S]*?padding:\s*0\.625rem\s+0\.75rem/i.test(breakpoint374Content)) {
    ...
    if (/\.form-navigation[\s\S]*?gap:\s*0\.75rem/i.test(breakpoint374Content)) {
...
const focusAt768 = /@media\s*\(max-width:\s*768px\)[\s\S]*?outline-width:\s*3px/i.test(cssContent);
const focusAt480 = /@media\s*\(max-width:\s*480px\)[\s\S]*?outline-width:\s*3px/i.test(cssContent);

if (focusAt768) {
    testPass('Focus enhancement applied at 768px breakpoint');
} else {
    testFail('Focus enhancement not found at 768px', 'Should enhance focus at tablet breakpoint');
}

if (focusAt480) {
    testWarn('Focus enhancement also at 480px', 'Ticket specifies 768px only - may cause duplication');
}

// Test 9: Focus outline-width is 3px
const focusWidthMatch = cssContent.match(/@media\s*\(max-width:\s*768px\)[\s\S]*?outline-width:\s*(\d+)px/i);
if (focusWidthMatch && focusWidthMatch[1] === '3') {
    testPass('Focus outline-width: 3px on mobile');
} else {
    testFail('Focus outline-width not 3px', `Found: ${focusWidthMatch ? focusWidthMatch[1] : 'none'}`);
}

// Test 10: Focus outline-offset is 3px
const focusOffsetMatch = cssContent.match(/@media\s*\(max-width:\s*768px\)[\s\S]*?outline-offset:\s*(\d+)px/i);
if (focusOffsetMatch && focusOffsetMatch[1] === '3') {
    testPass('Focus outline-offset: 3px on mobile');
} else {
    testFail('Focus outline-offset not 3px', `Found: ${focusOffsetMatch ? focusOffsetMatch[1] : 'none'}`);
}

// Test 11: Desktop focus still 2px (no regression)
const desktopFocusMatch = cssContent.match(/\.vas-dinamico-form\s*\*:focus-visible[\s\S]*?outline:\s*(\d+)px/i);
...
const breakpoint768Match = cssContent.match(/@media\s*\(max-width:\s*768px\)\s*\{([\s\S]*?)(?=@media|\/\*\s*===|$)/i);
if (breakpoint768Match) {
    ...
    if (foundControls >= 4) {
        testPass(`Specific interactive controls have enhanced focus (${foundControls}/${interactiveControls.length})`);
    } else {
        testWarn(`Only ${foundControls} interactive controls specified`, 'Consider adding more explicit selectors');
    }
}
...
const buttonPadding320 = /\.eipsi-prev-button[\s\S]*?padding:\s*0\.875rem\s+1\.5rem/i.test(cssContent);
if (buttonPadding320) {
    testPass('Navigation buttons maintain ~44px height at 320px (0.875rem padding)');
} else {
    testWarn('Button padding at 320px not verified', 'Check manually: 0.875rem × 2 + font + line-height ≥ 44px');
}

// Test 14: Radio/checkbox list items touch target
const radioListPadding = /\.radio-list li[\s\S]*?padding:\s*0\.75rem\s+0\.875rem/i.test(cssContent);
if (radioListPadding) {
...
// Check 320px padding
if (/@media\s*\(max-width:\s*374px\)[\s\S]*?\.vas-dinamico-form[\s\S]*?padding:\s*0\.75rem/i.test(cssContent)) {
...
// Check 480px padding
if (/@media\s*\(max-width:\s*480px\)[\s\S]*?\.vas-dinamico-form[\s\S]*?padding:\s*1rem/i.test(cssContent)) {
...
const focusColorMatch = cssContent.match(/outline:\s*\d+px\s+solid\s+var\(--eipsi-color-primary,\s*#([0-9a-fA-F]{6})\)/);
if (focusColorMatch) {
    const focusColor = focusColorMatch[1].toLowerCase();
    if (focusColor === '005a87') {
        testPass('Focus outline uses EIPSI blue #005a87 (WCAG AA compliant - 7.47:1)');
    } else {
        testWarn(`Focus color is #${focusColor}`, 'Should use #005a87 for brand consistency');
    }
}
...
if (/:focus-visible/i.test(cssContent)) {
    ...
    testFail(':focus-visible not found', 'Required for keyboard navigation detection');
}
