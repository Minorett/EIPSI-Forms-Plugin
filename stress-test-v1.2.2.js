...
const CONFIG = {
	...
	siteUrl:
		args.url || process.env.WP_SITE_URL || 'http://localhost/wordpress',
...
function log( message, color = 'reset' ) {
	...
	console.log(
		`${ colors[ color ] }[${ timestamp }] ${ message }${ colors.reset }`
	);
}
...
try {
	...
	console.error(
		`${ colors.red }ERROR: axios package not found. Please install it:${ colors.reset }`
	);
...
function generateFormData( options = {} ) {
	...
	const data = {
		...
		screen_width: [ 1920, 1440, 1366, 768, 375 ][
			Math.floor( Math.random() * 5 )
		],
		participant_id: `STRESS-${ Date.now() }-${ Math.random()
			.toString( 36 )
			.substr( 2, 9 ) }`,
			...
			if ( multipleChoice && field === 1 ) {
				...
				for ( let i = 0; i < numSelected; i++ ) {
					selectedOptions.push(
						`Option ${
							Math.floor(
								Math.random() * multipleChoiceOptions
							) + 1
						}`
					);
				}
				...
				data[ fieldName ] = 'Lorem ipsum dolor sit amet. '.repeat(
					200
				); // ~5400 chars
			} else {
				// Regular field
				data[ fieldName ] = `Response ${ Math.random()
					.toString( 36 )
					.substr( 2, 9 ) }`;
			}
		}
	...
	try {
		...
		if ( response.data && response.data.success ) {
			...
			if ( response.data.data && response.data.data.insert_id ) {
				results.metrics.submissionIds.push(
					response.data.data.insert_id
				);
			}
			return {
				success: true,
...
async function testMultipleSubmissions() {
	...
	for ( let i = 0; i < CONFIG.sequentialForms; i++ ) {
		log(
			`Submitting form ${ i + 1 }/${ CONFIG.sequentialForms }...`,
			'blue'
		);
	...
	const avgTime =
		sequentialResults.reduce( ( sum, r ) => sum + r.responseTime, 0 ) /
	...
	test( 'All 10 forms submitted successfully', {
		...
		message:
			successCount < CONFIG.sequentialForms
				? `Only ${ successCount }/${ CONFIG.sequentialForms } succeeded`
				: '',
	} );
	...
	test( 'Average response time < 2 seconds', {
		...
		warning:
			avgTime >= CONFIG.maxResponseTime &&
			avgTime < CONFIG.maxResponseTime * 1.5,
		message:
			avgTime >= CONFIG.maxResponseTime
	...
	test( 'All submission IDs are unique', {
		success:
			new Set( results.metrics.submissionIds ).size ===
			results.metrics.submissionIds.length,
		message:
			results.metrics.submissionIds.length -
				new Set( results.metrics.submissionIds ).size >
			0
	...
	subsection(
		'Test 1.2: Near-Simultaneous Submissions (5 forms < 100ms apart)'
	);
	...
	test( 'All 5 rapid forms submitted successfully', {
		...
		message:
			rapidSuccess < CONFIG.rapidForms
				? `Only ${ rapidSuccess }/${ CONFIG.rapidForms } succeeded`
				: '',
	} );
	...
	test( 'Rapid submissions completed in < 5 seconds total', {
		...
		message:
			rapidTime >= 5000
				? `Took ${ ( rapidTime / 1000 ).toFixed( 1 ) }s`
				: '',
	} );

	test( 'No duplicates in rapid submissions', {
		success:
			new Set( rapidResults.map( ( r ) => r.insertId ).filter( Boolean ) )
				.size === rapidSuccess,
		message: '',
	} );
	...
	for ( let i = 0; i < CONFIG.sustainedForms; i++ ) {
		...
		log(
			`  Submitted ${ i + 1 }/${ CONFIG.sustainedForms } (${
				result.responseTime
			}ms)`,
			'blue'
		);
	...
	const avgSustained =
		sustainedTimes.reduce( ( sum, t ) => sum + t, 0 ) /
		sustainedTimes.length;
	...
	test( 'All 20 sustained forms submitted successfully', {
		...
		message:
			sustainedSuccess < CONFIG.sustainedForms
				? `Only ${ sustainedSuccess }/${ CONFIG.sustainedForms } succeeded`
				: '',
	} );
	...
	test( 'Zero timeouts during sustained load', {
		...
		message:
			results.summary.timeouts > 0
				? `${ results.summary.timeouts } timeouts occurred`
				: '',
	} );

	// Update summary metrics
	results.summary.avgResponseTime =
		results.metrics.responseTimes.reduce( ( sum, t ) => sum + t, 0 ) /
		results.metrics.responseTimes.length;
	results.summary.maxResponseTime = Math.max(
		...results.metrics.responseTimes
	);
...
async function testComplexForms() {
	...
	test( 'Large form submission < 60 seconds', {
		...
		message:
			largeTime >= 60000
				? `Took ${ ( largeTime / 1000 ).toFixed( 1 ) }s`
				: '',
	} );
	...
	test( 'Large form response time < 5 seconds', {
		...
		warning:
			largeResult.responseTime >= 5000 &&
			largeResult.responseTime < 10000,
		message:
			largeResult.responseTime >= 5000
				? `${ largeResult.responseTime }ms`
				: '',
	} );

	// 2.2: Complex multiple choice
	subsection(
		'Test 2.2: Complex Multiple Choice (20+ options, 10 selected)'
	);
	...
	test( 'Large text response time acceptable', {
		...
		warning:
			largeTextResult.responseTime >= CONFIG.maxResponseTime * 2 &&
			largeTextResult.responseTime < CONFIG.maxResponseTime * 3,
		message:
			largeTextResult.responseTime >= CONFIG.maxResponseTime * 2
				? `${ largeTextResult.responseTime }ms`
				: '',
	} );
}
...
async function testMetadataStress() {
	...
	test( 'All 20 forms with metadata submitted successfully', {
		...
		message:
			metadataSuccess < 20
				? `Only ${ metadataSuccess }/20 succeeded`
				: '',
	} );
	...
	test( 'All forms captured required metadata', {
		...
		warning: true,
		message:
			'Database verification required (cannot verify without DB access)',
	} );
...
async function testDatabaseStress() {
	...
	log(
		'Submitting 20 forms in 1 minute to test connection stability...',
		'blue'
	);
	...
	const dbStressTimeouts = dbStressResults.filter(
	...
	test( 'All 20 database queries succeeded', {
		...
		message:
			dbStressSuccess < 20
				? `Only ${ dbStressSuccess }/20 succeeded`
				: '',
	} );
	...
	test( 'Zero connection timeouts', {
		...
		message:
			dbStressTimeouts > 0
				? `${ dbStressTimeouts } timeouts occurred`
				: '',
	} );
	...
	test( 'Average query time < 100ms', {
		...
		message:
			avgQueryTime >= 100
				? `Average: ${ avgQueryTime.toFixed( 0 ) }ms`
				: '',
	} );
	...
	test( 'No data loss detected', {
		...
		message:
			results.summary.dataLoss > 0
				? `${ results.summary.dataLoss } records lost`
				: '',
	} );
	...
	test( 'No duplicate records', {
		...
		message:
			results.summary.duplicates > 0
				? `${ results.summary.duplicates } duplicates found`
				: '',
	} );
}
...
async function testResourceMonitoring() {
	...
	log(
		`Baseline memory: ${ ( memoryBaseline.heapUsed / 1024 / 1024 ).toFixed(
			2
		) } MB`,
		'blue'
	);
	...
	log(
		`Final memory: ${ ( memoryFinal.heapUsed / 1024 / 1024 ).toFixed(
			2
		) } MB`,
		'blue'
	);
	...
	test( 'Memory growth < 10MB', {
		...
		warning:
			memoryGrowth >= CONFIG.maxMemoryGrowth &&
			memoryGrowth < CONFIG.maxMemoryGrowth * 2,
		message:
			memoryGrowth >= CONFIG.maxMemoryGrowth
				? `Grew ${ ( memoryGrowth / 1024 / 1024 ).toFixed( 2 ) } MB`
				: '',
	} );
	...
	test( 'CPU monitoring', {
		...
		warning: true,
		message:
			'CPU monitoring requires external tools (cannot measure from Node.js)',
	} );
	...
	test( 'System remains responsive under load', {
		success:
			responsiveResult.success && responsiveTime < CONFIG.maxResponseTime,
		message: ! responsiveResult.success
			? 'Request failed'
			: responsiveTime >= CONFIG.maxResponseTime
			? `${ responsiveTime }ms`
			: '',
	} );
...
function generateMetricsReport() {
	...
	console.log(
		`  Average (${
			results.summary.totalForms
		} forms): ${ results.summary.avgResponseTime.toFixed( 0 ) }ms`
	);
	...
	console.log(
		`  Forms per minute: ${ (
			results.summary.totalForms /
			( ( Date.now() - results.timestamp ) / 60000 )
		).toFixed( 2 ) }`
	);
	console.log(
		`  Successful submissions: ${ results.summary.successfulForms }/${ results.summary.totalForms }`
	);
	console.log( `  Failed submissions: ${ results.summary.failedForms }` );

	const memoryGrowth =
		results.metrics.memoryUsage.length > 0
			? results.metrics.memoryUsage[
					results.metrics.memoryUsage.length - 1
			  ].heapUsed - results.metrics.memoryUsage[ 0 ].heapUsed
			: 0;
	...
	console.log(
		`  Peak memory: ${
			results.metrics.memoryUsage.length > 0
				? (
						Math.max(
							...results.metrics.memoryUsage.map(
								( m ) => m.heapUsed
							)
						) /
						1024 /
	...
	const criteria = [
		{
			name: '20 forms without errors',
			passed: results.summary.successfulForms >= 20,
		},
		{
			name: 'Average time < 2 seconds',
			passed: results.summary.avgResponseTime < 2000,
		},
		{
			name: 'Memory usage stable',
			passed: memoryGrowth < CONFIG.maxMemoryGrowth,
		},
...
**Test Duration:** ${ (
		( Date.now() - new Date( results.timestamp ).getTime() ) /
		1000 /
		60
	).toFixed( 1 ) } minutes
...
- **Forms per Minute:** ${ (
		results.summary.totalForms /
		( ( Date.now() - new Date( results.timestamp ).getTime() ) / 60000 )
	).toFixed( 2 ) }
		...
		results.metrics.memoryUsage.length > 0
			? (
					( results.metrics.memoryUsage[
						...
						results.metrics.memoryUsage[ 0 ].heapUsed ) /
					1024 /
					1024
			  ).toFixed( 2 )
			: 0
...
| Criterion | Status |
|-----------|--------|
| 20 forms without errors | ${
		results.summary.successfulForms >= 20 ? '✅ PASS' : '❌ FAIL'
	} |
| Average time < 2 seconds | ${
		results.summary.avgResponseTime < 2000 ? '✅ PASS' : '❌ FAIL'
	} |
| Memory usage stable | ${
		results.metrics.memoryUsage.length > 0 &&
		results.metrics.memoryUsage[ results.metrics.memoryUsage.length - 1 ]
			.heapUsed -
			results.metrics.memoryUsage[ 0 ].heapUsed <
			CONFIG.maxMemoryGrowth
			? '✅ PASS'
			: '❌ FAIL'
	} |
...
${
	results.metrics.errors.length > 0
...
async function main() {
	...
	console.log(
		'║              EIPSI FORMS - PERFORMANCE STRESS TEST v1.2.2                  ║'
	);
	console.log(
		'║                                                                            ║'
	);
	...
	try {
		...
		log(
			'\nℹ️  This test suite requires a live WordPress installation.',
			'yellow'
		);
		log( '   To run the tests:', 'yellow' );
		log( '   1. Install EIPSI Forms v1.2.2 on a WordPress site', 'yellow' );
		log(
			'   2. Run: node stress-test-v1.2.2.js --url=https://your-site.com',
			'yellow'
		);
		log(
			'\nAlternatively, see STRESS_TEST_GUIDE.md for manual testing instructions.',
			'yellow'
		);
		process.exit( 1 );
	}
